...
class IPB_without(nn.Module):
    def __init__(self, in_dim=8, out_dim=4, dim=4, is_train=True):
        ...
        self.up1_cross = FAB_TSAB_inter(dim=dim, sq_dim=dim, dim_head=dim, mult=4)
        self.up1_action = StageInteraction(dim=dim)
        # self.downReuse1 = lightweight_module(dim, dim, is_train)
        # self.upReuse1 = lightweight_module(dim, dim, is_train)
    ...
    def forward(self, x, stage_ins=None, skip=False, stage_outs=None):
        ...
        if not skip:
            ...
            reuse = self.fusion_reuse(torch.cat([stage_ins[0], stage_ins[1]], dim=1))
            # x1 = self.down1(x)
            # x1 = self.downReuse1(x, stage_ins[0])
            x1 = self.down1_cross(x, stage_ins[0])
            x1 = self.down1_action(x1, stage_ins[0], stage_ins[0])
            stage_outs.append(x1)

            x = x1 + self.fusion1(torch.cat([reuse, x1], dim=1))
            # x = self.up1(x)
            x = self.up1_cross(x, stage_ins[1])
...
class NetVideo_base_noStageInteraction_deepcache_action_ffnlw_single_multireuse_multimain_replace_new_normalunfolding_cross_17(
    torch.nn.Module):
    def __init__(self, opt):
        super(
            NetVideo_base_noStageInteraction_deepcache_action_ffnlw_single_multireuse_multimain_replace_new_normalunfolding_cross_17,
            self).__init__()
