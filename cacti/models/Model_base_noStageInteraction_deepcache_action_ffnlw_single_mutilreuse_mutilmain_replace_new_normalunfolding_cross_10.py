...
class FA_inter(nn.Module):
    ...
    def cal_attention(self, x, last):
        q = self.to_q(x)
        k_self = self.to_k_self(x)
        k_cross = self.to_k_cross(last)
        v_self = self.to_v_self(x)
        v_cross = self.to_v_cross(last)
        k_self = torch.cat([k_self, k_cross], dim=0)
        v_self = torch.cat([v_self, v_cross], dim=0)
        q, k_self = map(lambda t: rearrange(t, 'b n B (h d) -> b h B n d', h=self.heads_qk), (q, k_self))
        v_self = rearrange(v_self, 'b n B (h d) -> b h B n d', h=self.heads_v)
        attn = self.cal_atten(q, k_self)
        out = einsum('b h B i j, b h B j d -> b h B i d', attn, v_self)
        out = rearrange(out, 'b h B n d -> b n B (h d)')
        out = self.to_out(out)
        return out
...
class NetVideo_base_noStageInteraction_deepcache_action_ffnlw_single_multireuse_multimain_replace_new_normalunfolding_cross_10(
    torch.nn.Module):
    def __init__(self, opt):
        super(
            NetVideo_base_noStageInteraction_deepcache_action_ffnlw_single_multireuse_multimain_replace_new_normalunfolding_cross_10,
            self).__init__()
