...
class StageInteraction(nn.Module):
    def __init__(self, dim):
        ...
        # self.phi2 = nn.Conv3d(8, 8, (3, 1, 1), 1, (1, 0, 0), bias=False, groups=8)
        self.gamma1 = nn.Conv3d(dim, dim, (1, 3, 3), 1, (0, 1, 1), bias=False, groups=dim)
        # self.gamma2 = nn.Conv3d(8, 8, (3, 1, 1), 1, (1, 0, 0), bias=False, groups=8)

    def forward(self, inp, pre_enc, pre_dec):
        pre_enc = inp
        out = self.st_inter_enc(pre_enc) + self.st_inter_dec(pre_dec)
        skip = self.act_fn(out)
        phi = torch.sigmoid(self.phi1(skip))
        # skip2 = skip.permute(0, 2, 1, 3, 4)
        # phi = torch.sigmoid(self.phi1(skip) + self.phi2(skip2).permute(0, 2, 1, 3, 4))
        gamma = self.gamma1(skip)
...
class NetVideo_base_noStageInteraction_deepcache_action_ffnlw_single_multireuse_multimain_replace_new(torch.nn.Module):
    def __init__(self, opt):
        super(NetVideo_base_noStageInteraction_deepcache_action_ffnlw_single_multireuse_multimain_replace_new,
              self).__init__()
        ...
        for i in range(len(opt.reuse)):
            ...
            if j == 1:
                ...
                self.net.append(nn.ModuleList([
                    # IPB(in_dim=opt.dim * 2, out_dim=opt.dim, dim=opt.dim),
                    IPB_without(in_dim=opt.dim * 2, out_dim=opt.dim, dim=opt.dim),
                    # DPB(in_dim=opt.dim * 2, dim=opt.dim),
                    FB(in_dim=opt.dim * 2, dim=opt.dim)
                ]))
    ...
    def forward(self, g, input_mask=None):
        ...
        for i in range(len(self.reuse)):
            ...
            mu = self.mu[i](f)
            if j != 0:
                z, stage_output = self.net[i][0](torch.cat([f + y / mu, f], dim=1))
            else:
                z, stage_output = self.net[i][0](torch.cat([f + y / mu, f], dim=1), stage_output, True)
