...
class FeedForward_FAB(nn.Module):
    def __init__(self, dim, mult=4):
        super().__init__()
        self.net = nn.Sequential(
            nn.Conv3d(dim, dim * mult, 1, 1, bias=False),
...
class FAB(nn.Module):
    def __init__(self, dim, sq_dim, window_size=(8, 8), dim_head=28, mult=4, shift=False):
        ...
        self.norm2 = nn.LayerNorm(dim)
        # self.mpmlp = MPMLP(dim=dim, multi=mult)
        self.ffn = FeedForward_FAB(dim=dim, mult=mult)
...
class TSAB(nn.Module):
    def __init__(self, dim, num_head):
        ...
        self.ffn = FeedForward_FAB(dim=dim)
...
class NetVideo_base_ffnchange2(torch.nn.Module):
    def __init__(self, opt):
        super(NetVideo_base_ffnchange2, self).__init__()
