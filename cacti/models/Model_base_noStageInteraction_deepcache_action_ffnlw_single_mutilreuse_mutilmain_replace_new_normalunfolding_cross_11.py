...
class FA_inter(nn.Module):
    def __init__(self, dim, window_size=(8, 8), dim_head=28, sq_dim=None, shift=True):
        ...
        num_token = window_size[0] * window_size[1]
        self.cal_atten_x_last = Attention(dim_head, num_token)
        self.cal_atten_last1_x = Attention(dim_head, num_token)
        self.cal_atten_last2_x = Attention(dim_head, num_token)
        self.to_q = nn.Linear(dim, dim, bias=False)
        self.to_v = nn.Linear(dim, dim, bias=False)
        self.to_q_last2 = nn.Linear(dim, dim, bias=False)
        self.to_v_last2 = nn.Linear(dim, dim, bias=False)
        self.to_q_last1 = nn.Linear(dim, dim, bias=False)
        self.to_v_last1 = nn.Linear(dim, dim, bias=False)
        self.to_out_x = nn.Linear(dim, dim)
        self.to_out_last1 = nn.Linear(dim, dim)
        self.to_out_last2 = nn.Linear(dim, dim)
        # self.fusion = nn.Conv3d(dim*2, dim, kernel_size=1, stride=1, padding=0)

    def cal_attention(self, x, last1, last2):
        q = self.to_q(x)
        v = self.to_v(x)
        q_last1 = self.to_q_last1(last1)
        v_last1 = self.to_v_last1(last1)
        q_last2 = self.to_q_last2(last2)
        v_last2 = self.to_v_last2(last2)
        q, v, q_last1, v_last1, q_last2, v_last2 = map(lambda t: rearrange(t, 'b n B (h d) -> b h B n d', h=self.heads_qk), (q, v, q_last1, v_last1, q_last2, v_last2))
        attn_x_last = self.cal_atten_x_last(q, q_last1 + q_last2)
        attn_last1_x = self.cal_atten_last1_x(q_last1, q)
        attn_last2_x = self.cal_atten_last2_x(q_last2, q)
        out_x = einsum('b h B i j, b h B j d -> b h B i d', attn_x_last, v)
        out_x = rearrange(out_x, 'b h B n d -> b n B (h d)')
        out_x = self.to_out_x(out_x)
        out_last1 = einsum('b h B i j, b h B j d -> b h B i d', attn_last1_x, v_last1)
        out_last1 = rearrange(out_last1, 'b h B n d -> b n B (h d)')
        out_last1 = self.to_out_last1(out_last1)
        out_last2 = einsum('b h B i j, b h B j d -> b h B i d', attn_last2_x, v_last2)
        out_last2 = rearrange(out_last2, 'b h B n d -> b n B (h d)')
        out_last2 = self.to_out_last2(out_last2)
        return out_x, out_last1, out_last2

    def forward(self, x, last1, last2):
        b, B, h, w, c = x.shape
        w_size = self.window_size
        x_inp = rearrange(x, 'b B (h b0) (w b1) c -> (b h w) (b0 b1) B c', b0=w_size[0], b1=w_size[1])
        last1 = rearrange(last1, 'b B (h b0) (w b1) c -> (b h w) (b0 b1) B c', b0=w_size[0], b1=w_size[1])
        last2 = rearrange(last2, 'b B (h b0) (w b1) c -> (b h w) (b0 b1) B c', b0=w_size[0], b1=w_size[1])
        out, out_last1, out_last2 = self.cal_attention(x_inp, last1, last2)
        out = rearrange(out, '(b h w) (b0 b1) B c -> b B (h b0) (w b1) c', h=h // w_size[0], w=w // w_size[1],
                        b0=w_size[0])
        out_last1 = rearrange(out_last1, '(b h w) (b0 b1) B c -> b B (h b0) (w b1) c', h=h // w_size[0], w=w // w_size[1],
                        b0=w_size[0])
        out_last2 = rearrange(out_last2, '(b h w) (b0 b1) B c -> b B (h b0) (w b1) c', h=h // w_size[0], w=w // w_size[1],
                        b0=w_size[0])
        if self.shift:
            out = out.roll(shifts=-4, dims=2).roll(shifts=-4, dims=3)
        return out, out_last1, out_last2
...
class FAB_inter(nn.Module):
    ...
    def forward(self, x, last1, last2):
        ...
        # x = x.permute(0, 2, 3, 1)  # 把通道放在后面
        x = x.permute(0, 2, 3, 4, 1)  # 把通道放在后面
        last1 = last1.permute(0, 2, 3, 4, 1)  # 把通道放在后面
        last2 = last2.permute(0, 2, 3, 4, 1)  # 把通道放在后面
        x_ = self.norm1(x)
        last1_ = self.norm1_last(last1)
        last2_ = self.norm2_last(last2)
        x1, last1, last2 = self.fa(x_, last1_, last2_)
        x = x_ + x1
        last1 = last1 + last1_
        last2 = last2 + last1_
        # x_ = self.norm2(x)
        # x = self.ffn(x_) + x
        x = x.permute(0, 4, 1, 2, 3)
        last1 = last1.permute(0, 4, 1, 2, 3)
        last2 = last2.permute(0, 4, 1, 2, 3)
        return x, last1, last2
...
class FAB_TSAB_inter(nn.Module):
    ...
    def forward(self, x, last1, last2):
        x, last1, last2 = self.FAB_inter(x, last1, last2)
        x = self.TSAB_inter(x)
        # x = self.TSAB2(x)
        return x, last1, last2
...
class TimesAttention3D_inter(nn.Module):
    ...
    def forward(self, x):
        ...
        _, _, _, h, w = x.shape
        # last_tsab_in = self.fusion(torch.cat([last1, last2], dim=1))
        tsab_in = rearrange(x, "b B c h w->(b h w) B c")
        # last_tsab_in = rearrange(tsab_in, "b c B h w->(b h w) B c")
        n, B, C = tsab_in.shape
        q = self.q(tsab_in)
        kv = self.kv(tsab_in)
        q = q.reshape(n, B, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        kv = kv.reshape(-1, B, 2, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        # C = C // 2
        k, v = kv[0], kv[1]
...
class TSAB_inter(nn.Module):
    ...
    def forward(self, x):
        ...
        # last1 = self.norm1_last(last1.permute(0, 2, 3, 4, 1)).permute(0, 4, 1, 2, 3)
        # last2 = self.norm1_last(last2.permute(0, 2, 3, 4, 1)).permute(0, 4, 1, 2, 3)
        x1 = self.tsab_inter(x_)
        x = x_ + x1
        # x = x.permute(0, 2, 3, 4, 1)  # 把通道放在后面
        # x = self.ffn(self.norm2(x)) + x
        # x = x.permute(0, 4, 1, 2, 3)
...
class IPB_without(nn.Module):
    def __init__(self, in_dim=8, out_dim=4, dim=4, is_train=True):
        ...
        # # self.fusion1 = nn.Conv2d(dim * 2, dim, 1, 1, 0, bias=False)
        # # 改动
        self.fusion1 = nn.Conv3d(dim * 3, dim, 1, 1, 0, bias=False)
        # self.up1 = FAB_TSAB(dim=dim, sq_dim=dim, dim_head=dim, mult=4, shift=True)
        # self.conv_out = nn.Conv2d(dim, out_dim, 3, 1, 1, bias=False)
        # 改动
        # self.conv_out = nn.Conv3d(dim, out_dim, 3, 1, 1, bias=False)
    ...
    def forward(self, x, stage_ins=None, skip=False, stage_outs=None):
        ...
        if not skip:
            ...
            x1 = self.down1(x)
            # x1 = self.downReuse1(x, stage_ins[0])
            x1, last1, last2 = self.down1_cross(x1, stage_ins[0], stage_ins[1])
            x1 = self.down1_action(x1, stage_ins[0], stage_ins[1])
            stage_outs.append(last1)

            x = x1 + self.fusion1(torch.cat([stage_ins[0], stage_ins[1], x1], dim=1))
            # x = self.up1(x)
            # x = self.upReuse1(x, stage_ins[1])
            x = self.up1_action(x, stage_ins[0], stage_ins[1])
            stage_outs.append(last2)
...
class NetVideo_base_noStageInteraction_deepcache_action_ffnlw_single_multireuse_multimain_replace_new_normalunfolding_cross_11(
    torch.nn.Module):
    def __init__(self, opt):
        super(
            NetVideo_base_noStageInteraction_deepcache_action_ffnlw_single_multireuse_multimain_replace_new_normalunfolding_cross_11,
            self).__init__()
