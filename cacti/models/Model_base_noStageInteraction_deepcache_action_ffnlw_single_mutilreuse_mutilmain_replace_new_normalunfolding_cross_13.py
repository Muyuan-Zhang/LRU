...
class IPB_without(nn.Module):
    ...
    def forward(self, x, stage_ins=None, skip=False, stage_outs=None):
        ...
        if not skip:
            ...
            x1 = x1 + self.fusion1(torch.cat([stage_ins[0], x1], dim=1))
            x1 = self.down1_cross(x1, stage_ins[1])
            x1 = self.down1_action(x1, stage_ins[0], stage_ins[1])
            stage_outs.append(x1)

            x = x1 + self.fusion2(torch.cat([stage_ins[1], x1], dim=1))
            # x = self.up1(x)
            # x = self.upReuse1(x, stage_ins[1])
            x = self.up1_action(x, stage_ins[1], stage_ins[0])
            stage_outs.append(x)
...
class NetVideo_base_noStageInteraction_deepcache_action_ffnlw_single_multireuse_multimain_replace_new_normalunfolding_cross_13(
    torch.nn.Module):
    def __init__(self, opt):
        super(
            NetVideo_base_noStageInteraction_deepcache_action_ffnlw_single_multireuse_multimain_replace_new_normalunfolding_cross_13,
            self).__init__()
