...
class FAB(nn.Module):
    ...
    def forward(self, x):
        ...
        x_ = self.norm1(x)
        x = self.fa(x_) + x
        x_ = self.norm2(x.permute(0, 4, 1, 2, 3)).permute(0, 2, 3, 4, 1)
        x = self.ffn(x_) + x
        x = x.permute(0, 4, 1, 2, 3)
        return x
...
class TSAB(nn.Module):
    ...
    def forward(self, x):
        x = self.tsab(x) + x
        x = x.permute(0, 2, 3, 4, 1)  # 把通道放在后面
        x = self.ffn(self.norm2(x.permute(0, 4, 1, 2, 3)).permute(0, 2, 3, 4, 1)) + x
        x = x.permute(0, 4, 1, 2, 3)
        return x
...
class NetVideo_base_noStageInteraction_deepcache_action_ffnlw_single_multireuse_multimain_replace_new_normalunfolding_cross_54(
    torch.nn.Module):
    def __init__(self, opt):
        super(
            NetVideo_base_noStageInteraction_deepcache_action_ffnlw_single_multireuse_multimain_replace_new_normalunfolding_cross_54,
            self).__init__()
