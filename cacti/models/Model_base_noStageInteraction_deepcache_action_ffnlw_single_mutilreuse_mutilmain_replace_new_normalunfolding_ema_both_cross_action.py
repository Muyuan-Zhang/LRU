...
class IPB_without(nn.Module):
    def __init__(self, in_dim=8, out_dim=4, dim=4, is_train=True):
        ...
        # self.conv_in = nn.Conv3d(in_dim, dim, 3, 1, 1, bias=False)
        # self.conv_in = nn.Conv3d(in_dim, dim, 1, 1, 0, bias=False)
        self.down1 = FAB_TSAB(dim=dim, sq_dim=dim, dim_head=dim, mult=4, shift=True)
        self.down2 = FAB_TSAB_inter(dim=dim, sq_dim=dim, dim_head=dim, mult=4)
        # # self.downsample1 = nn.Conv2d(dim, dim * 2, 4, 2, 1, bias=False)
        # # 改动
        # # self.downsample1 = nn.Conv3d(dim, dim * 2, 4, 2, 1, bias=False)
        # # 改变下采样，帧数保持不变
...
class NetVideo_base_noStageInteraction_deepcache_action_ffnlw_single_multireuse_multimain_replace_new_normalunfolding_ema_both_cross_attention(
    torch.nn.Module):
    def __init__(self, opt):
        super(
            NetVideo_base_noStageInteraction_deepcache_action_ffnlw_single_multireuse_multimain_replace_new_normalunfolding_ema_both_cross_attention,
            self).__init__()
