...
class Conv3d(nn.Module):
    def __init__(self, dim):
        super().__init__()
        self.conv = nn.Conv3d(dim, dim, kernel_size=1, stride=1, padding=0)
    def forward(self, x):
        return self.conv(x)
...
class NetVideo_base_noStageInteraction_deepcache_action_ffnlw_single_multireuse_multimain_replace_new_normalunfolding_cross_29_test(
    torch.nn.Module):
    def __init__(self, opt):
        super(
            NetVideo_base_noStageInteraction_deepcache_action_ffnlw_single_multireuse_multimain_replace_new_normalunfolding_cross_29_test,
            self).__init__()
        ...
        # 改动：添加一个特征提取块
        self.fem1 = FEM(in_dim=1, dim=self.dim // 4)
    ...
    def forward(self, g, input_mask=None):
        ...
        # 直接特征提取
        temp_g = self.fem1(temp_g.permute(0, 4, 1, 2, 3))  # 形状[bs, c 8, h, w]
        g = g.permute(0, 4, 1, 2, 3)  # 形状[bs, c 8, h, w]
        Phi = self.fem2(Phi.permute(0, 4, 1, 2, 3))  # 形状[bs, c 8, h, w]
