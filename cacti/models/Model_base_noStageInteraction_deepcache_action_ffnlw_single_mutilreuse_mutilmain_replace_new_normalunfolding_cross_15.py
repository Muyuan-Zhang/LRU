...
class CrossForward(nn.Module):
    def __init__(self,
        ...
        super(CrossForward, self).__init__()

        hidden_features = int(dim * ffn_expansion_factor)

        self.project_in_x = nn.Conv3d(dim, hidden_features, kernel_size=1, bias=bias)
        self.project_in_last = nn.Conv3d(dim, hidden_features, kernel_size=1, bias=bias)

        self.dwconv_x = nn.Conv3d(hidden_features, hidden_features, kernel_size=3, stride=1, padding=1,
                                  groups=hidden_features, bias=bias)
        self.dwconv_last = nn.Conv3d(hidden_features, hidden_features, kernel_size=3, stride=1, padding=1,
                                  groups=hidden_features, bias=bias)
        self.act_fn = GELU()

        self.project_out_x = nn.Conv3d(hidden_features, dim, kernel_size=1, bias=bias)
        self.project_out_last = nn.Conv3d(hidden_features, dim, kernel_size=1, bias=bias)

    def forward(self, x, last):
        x = self.project_in_x(x)
        last = self.project_in_last(last)
        x = self.dwconv_x(x)
        last = self.dwconv_last(last)
        x_ = self.act_fn(last) * x
        last_ = self.act_fn(x) * last
        x = self.project_out_x(x_)
        last = self.project_out_last(last_)
        return x, last
...
class FA_inter(nn.Module):
    def __init__(self, dim, window_size=(8, 8), dim_head=28, sq_dim=None, shift=True):
        ...
        self.to_v_x = nn.Linear(dim, dim, bias=False)
        self.to_q_x = nn.Linear(dim, dim, bias=False)
        self.to_q_last = nn.Linear(dim, dim, bias=False)
        self.to_v_last = nn.Linear(dim, dim, bias=False)
        self.to_out_x = nn.Linear(dim, dim)
        self.to_out_last = nn.Linear(dim, dim)

    def cal_attention(self, x, last):
        q_x = self.to_q_x(x)
        q_last = self.to_q_last(last)
        v_x = self.to_v_x(x)
        v_last = self.to_v_last(last)
        q_x, q_last = map(lambda t: rearrange(t, 'b n B (h d) -> b h B n d', h=self.heads_qk), (q_x, q_last))
        v_x = rearrange(v_x, 'b n B (h d) -> b h B n d', h=self.heads_v)
        v_last = rearrange(v_last, 'b n B (h d) -> b h B n d', h=self.heads_v)
        attn = self.cal_atten(q_x, q_last)
        out_x = einsum('b h B i j, b h B j d -> b h B i d', attn, v_x)
        out_last = einsum('b h B i j, b h B j d -> b h B i d', attn, v_last)
        out_x = rearrange(out_x, 'b h B n d -> b n B (h d)')
        out_last = rearrange(out_last, 'b h B n d -> b n B (h d)')
        out_x = self.to_out_x(out_x)
        out_last = self.to_out_last(out_last)
        return out_x, out_last
    ...
    def forward(self, x, last):
        ...
        out, last = self.cal_attention(x_inp, last)
        out = rearrange(out, '(b h w) (b0 b1) B c -> b B (h b0) (w b1) c', h=h // w_size[0], w=w // w_size[1],
                        b0=w_size[0])
        last = rearrange(last, '(b h w) (b0 b1) B c -> b B (h b0) (w b1) c', h=h // w_size[0], w=w // w_size[1],
                         b0=w_size[0])
        if self.shift:
            out = out.roll(shifts=-4, dims=2).roll(shifts=-4, dims=3)
        return out, last
...
class FAB_inter(nn.Module):
    def __init__(self, dim, sq_dim, window_size=(8, 8), dim_head=28, mult=4, shift=False):
        ...
        self.norm2 = nn.LayerNorm(dim)
        self.norm2_last = nn.LayerNorm(dim)
        # self.mpmlp = MPMLP(dim=dim, multi=mult)
        self.ffn = CrossForward(dim)
    ...
    def forward(self, x, last):
        ...
        x_ = self.norm1(x)
        last_ = self.norm1_last(last)
        x_, last_ = self.fa(x_, last_)
        x = x_ + x
        last = last_ + last
        x_ = self.norm2(x)
        last_ = self.norm2_last(last)
        x_ = x_.permute(0, 4, 1, 2, 3)
        last_ = last_.permute(0, 4, 1, 2, 3)
        x_, last_ = self.ffn(x_, last_)
        x_ = x_.permute(0, 2, 3, 4, 1)
        last_ = last_.permute(0, 2, 3, 4, 1)
        x = x + x_
        last = last + last_
        x = x.permute(0, 4, 1, 2, 3)
        last = last.permute(0, 4, 1, 2, 3)
        return x, last
...
class FAB_TSAB_inter(nn.Module):
    ...
    def forward(self, x, last):
        x, last = self.FAB_inter(x, last)
        x, last = self.TSAB_inter(x, last)
        # x = self.TSAB2(x)
        return x, last
...
class TimesAttention3D_inter(nn.Module):
    def __init__(self, dim, num_head, qkv_bias=False, qk_scale=None):
        ...
        self.q_last = nn.Linear(dim, dim, bias=qkv_bias)
        self.v_last = nn.Linear(dim, dim, bias=qkv_bias)

        self.proj = nn.Linear(dim, dim)
        self.proj_last = nn.Linear(dim, dim)
    ...
    def forward(self, x, last):
        ...
        last_tsab_in = rearrange(last, "b c B h w->(b h w) B c")
        n, B, C = tsab_in.shape
        q_x = self.q_x(tsab_in)
        v_x = self.v_x(tsab_in)
        q_last = self.q_last(last_tsab_in)
        v_last = self.v_last(last_tsab_in)
        q_x = q_x.reshape(n, B, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        v_x = v_x.reshape(n, B, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        q_last = q_last.reshape(n, B, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        v_last = v_last.reshape(n, B, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        # C = C // 2
        q_x = q_x * self.scale
        attn = q_x @ q_last.transpose(-2, -1)
        # 无嵌入向量
        attn = self.softmax(attn)
        x = (attn @ v_x).transpose(1, 2).reshape(n, B, C)
        last = (attn @ v_last).transpose(1, 2).reshape(n, B, C)
        x = rearrange(x, "(b h w) B c->b B h w c", h=h, w=w)
        last = rearrange(last, "(b h w) B c->b B h w c", h=h, w=w)
        x = self.proj(x)
        last = self.proj_last(last)
        x = x.permute(0, 4, 1, 2, 3)
        last = last.permute(0, 4, 1, 2, 3)
        return x, last
...
class TSAB_inter(nn.Module):
    def __init__(self, dim, num_head):
        ...
        self.norm2_last = nn.LayerNorm(dim)
        # self.mpmlp = MPMLP(dim=dim, multi=4)
        self.ffn = CrossForward(dim=dim)

    def forward(self, x, last):
        x_ = self.norm1_x(x.permute(0, 2, 3, 4, 1)).permute(0, 4, 1, 2, 3)
        last_ = self.norm1_last(last.permute(0, 2, 3, 4, 1)).permute(0, 4, 1, 2, 3)
        x_, last_ = self.tsab_inter(x_, last_)
        x = x_ + x
        last = last_ + last

        # x = x.permute(0, 2, 3, 4, 1)  # 把通道放在后面
        # last = last.permute(0, 2, 3, 4, 1)  # 把通道放在后面
        x_, last_ = self.ffn(self.norm2_x(x.permute(0, 2, 3, 4, 1)).permute(0, 4, 1, 2, 3), self.norm2_last(last.permute(0, 2, 3, 4, 1)).permute(0, 4, 1, 2, 3))
        x = x + x_
        last = last_ + last
        # x = x.permute(0, 4, 1, 2, 3)
        # last = last.permute(0, 4, 1, 2, 3)
...
class IPB_without(nn.Module):
    def __init__(self, in_dim=8, out_dim=4, dim=4, is_train=True):
        ...
        # # self.fusion1 = nn.Conv2d(dim * 2, dim, 1, 1, 0, bias=False)
        # # 改动
        self.fusion_reuse = nn.Conv3d(dim * 2, dim, 1, 1, 0, bias=False)
        self.fusion2 = nn.Conv3d(dim * 2, dim, 1, 1, 0, bias=False)
        # self.up1 = FAB_TSAB(dim=dim, sq_dim=dim, dim_head=dim, mult=4, shift=True)
        # self.conv_out = nn.Conv2d(dim, out_dim, 3, 1, 1, bias=False)
        # 改动
        # self.conv_out = nn.Conv3d(dim, out_dim, 3, 1, 1, bias=False)
        ...
        # self.down1_action = StageInteraction(dim=dim)
        self.down1_cross = FAB_TSAB_inter(dim=dim, sq_dim=dim, dim_head=dim, mult=4)
    ...
    def forward(self, x, stage_ins=None, skip=False, stage_outs=None):
        ...
        if not skip:
            ...
            F_reuse = self.fusion_reuse(torch.cat([stage_ins[0], stage_ins[1]], dim=1))
            # x1 = self.down1(x)
            # x1 = self.downReuse1(x, x stage_ins[0])
            x1, F_reuse = self.down1_cross(x, F_reuse)
            # x1 = self.down1_action(x1, stage_ins[0], stage_ins[1])
            stage_outs.append(x1)

            x = x1 + self.fusion2(torch.cat([F_reuse, x1], dim=1))
            x, F_reuse = self.down2_cross(x, F_reuse)
            # x = self.up1(x)
            # x = self.upReuse1(x, stage_ins[1])
            # x = self.up1_action(x, stage_ins[1], stage_ins[0])
            stage_outs.append(x)
...
class NetVideo_base_noStageInteraction_deepcache_action_ffnlw_single_multireuse_multimain_replace_new_normalunfolding_cross_15(
    torch.nn.Module):
    def __init__(self, opt):
        super(
            NetVideo_base_noStageInteraction_deepcache_action_ffnlw_single_multireuse_multimain_replace_new_normalunfolding_cross_15,
            self).__init__()
