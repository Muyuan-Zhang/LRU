...
#################
# 输出用dw1x1x1，qkv用dwconv3x3x3 base23
# 试下阶段融合换下enc dec位置 base 26
# 第一个cross用enc的值 base29（对permute调整）
# 先不写到一起，测试下cross+SF，只写到TSAB里面 用v生成Phi和gamma 30出头  35
# 37 完全初始的SF
# 不用v生成就用last 36
# 38 将SF视为ffn，先用在tsab后面
# 39先不管性能做成空间+crossffn+fusion+时间+crossffn， 然后ffn的残差连接用上last
# 39 完全不行 去掉残差 没有大变化
# 39 去掉中间的fusion试下 更不行
# 39 两部分fusion 更不行
# 39 fusion融合enc dec好一点
# 39 fusion放前面
# 40 完全不行
# 41 从39复制，大体是空间+时间+crossffn（silu） fusion再考虑
# 42 41不行 修改初始化
#################
...
class CrossFFN(nn.Module):
    ...
    def forward(self, x, last):
        ...
        return out: [b,h,w,c]
        """
        out = self.out(self.x(x) * self.last(last))
        return out
...
class CrossFFNBlock(nn.Module):
    def __init__(self, dim, mult=1):
        ...
        self.ln_last = nn.LayerNorm(dim)
        self.ffn = CrossFFN(dim)

    def forward(self, x, last):
        x_ = self.ln_x(x.permute(0, 2, 3, 4, 1)).permute(0, 4, 1, 2, 3)
        last_ = self.ln_last(last.permute(0, 2, 3, 4, 1)).permute(0, 4, 1, 2, 3)
        out = x + self.ffn(x_, last_)
        return out
...
class FAB_TSAB_inter(nn.Module):
    def __init__(self, dim, sq_dim, window_size=(8, 8), dim_head=28, mult=4, shift=False):
        ...
        self.FAB_inter = FAB_inter(dim=dim, sq_dim=sq_dim, window_size=window_size, dim_head=dim_head, mult=mult,
                                   shift=shift)
        self.ffn1 = CrossFFNBlock(dim)
        self.TSAB_inter = TSAB_inter(dim, num_head=dim_head)
        # self.fusion1 = nn.Conv3d(dim * 2, dim, kernel_size=1, stride=1, padding=0)
        # self.fusion2 = nn.Conv3d(dim * 2, dim, kernel_size=1, stride=1, padding=0)
        # self.TSAB_inter = TSAB(dim, num_head=dim_head)
        # self.TSAB2 = TSAB(dim, num_head=dim_head)
        self.ffn2 = CrossFFNBlock(dim)

    def forward(self, x, enc, dec):
        # x = x + self.fusion1(torch.cat([x, enc, dec], dim=1))
        x = self.FAB_inter(x, enc)
        enc = self.ffn1(x, enc)

        x = self.TSAB_inter(enc, dec)

        # x = enc + self.fusion2(torch.cat([enc, dec], dim=1))
        # x = enc + self.fusion2(torch.cat([dec, enc], dim=1))
        x = self.ffn2(x, dec)
...
class IPB_without(nn.Module):
    def __init__(self, in_dim=8, out_dim=4, dim=4, is_train=True):
        ...
        # self.down1_action = StageInteraction(dim=dim)
        self.down1_cross = FAB_TSAB_inter(dim=dim, sq_dim=dim, dim_head=dim, mult=1)
    ...
    def forward(self, x, stage_ins=None, skip=False, stage_outs=None):
        ...
        if not skip:
            ...
            # x = self.down1(x)
            # x1 = self.downReuse1(x, stage_ins[0])
            x, enc, dec = self.down1_cross(x, stage_ins[0], stage_ins[1])
            # x1 = self.down1_action(x1, stage_ins[0], stage_ins[0])
            stage_outs.append(enc)
...
class NetVideo_base_noStageInteraction_deepcache_action_ffnlw_single_multireuse_multimain_replace_new_normalunfolding_cross_42(
    torch.nn.Module):
    def __init__(self, opt):
        super(
            NetVideo_base_noStageInteraction_deepcache_action_ffnlw_single_multireuse_multimain_replace_new_normalunfolding_cross_42,
            self).__init__()
        ...
        #     FB(in_dim=opt.dim * 2, dim=opt.dim)
        # ]))
        self.fusion1 = nn.Conv3d(self.dim * 2, self.dim, 1, 1, 0)
        self.fusion2 = nn.Conv3d(self.dim * 2, self.dim, 1, 1, 0)
        # 改动：添加一个特征提取块
    ...
    def forward(self, g, input_mask=None):
        ...
        for i in range(len(self.reuse)):
            ...
            if j != 0:
                ...
                enc = self.fusion1(torch.cat([stage_output[0], stage_output[2]], dim=1))
                dec = self.fusion1(torch.cat([stage_output[1], stage_output[3]], dim=1))
                out
