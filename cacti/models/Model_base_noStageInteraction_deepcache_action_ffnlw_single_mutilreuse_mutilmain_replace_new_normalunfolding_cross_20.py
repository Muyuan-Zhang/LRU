...
class FA_inter(nn.Module):
    def __init__(self, dim, window_size=(8, 8), dim_head=28, sq_dim=None, shift=True):
        ...
        self.to_v = nn.Conv2d(dim, dim, kernel_size=1, stride=1, padding=0, bias=False, groups=dim)
        self.to_k = nn.Conv2d(dim, dim, kernel_size=1, stride=1, padding=0, bias=False, groups=dim)
        self.to_q = nn.Conv2d(dim, dim, kernel_size=1, stride=1, padding=0, bias=False, groups=dim)
        # self.to_q = nn.Linear(dim, dim, bias=False)
        # self.to_k = nn.Linear(dim, dim, bias=False)
        self.to_out = nn.Linear(dim, dim)

    def cal_attention(self, x, last):
        q = self.to_q(x.permute(0, 3, 1, 2)).permute(0, 2, 3, 1)
        k = self.to_k(last.permute(0, 3, 1, 2)).permute(0, 2, 3, 1)
        v = self.to_v(last.permute(0, 3, 1, 2)).permute(0, 2, 3, 1)
        q, k = map(lambda t: rearrange(t, 'b n B (h d) -> b h B n d', h=self.heads_qk), (q, k))
        v = rearrange(v, 'b n B (h d) -> b h B n d', h=self.heads_v)
        attn = self.cal_atten(q, k)
        out = einsum('b h B i j, b h B j d -> b h B i d', attn, v)
    ...
    def forward(self, x, last):
        ...
        out = self.cal_attention(x_inp, last)
        out = rearrange(out, '(b h w) (b0 b1) B c -> b B (h b0) (w b1) c', h=h // w_size[0], w=w // w_size[1],
                        b0=w_size[0])
        if self.shift:
            out = out.roll(shifts=-4, dims=2).roll(shifts=-4, dims=3)
...
class TimesAttention3D_inter(nn.Module):
    def __init__(self, dim, num_head, qkv_bias=False, qk_scale=None):
        ...
        self.q = nn.Conv1d(dim, dim, 1, 1, bias=qkv_bias, groups=dim)
        self.kv = nn.Conv1d(dim, dim * 2, 1, 1, bias=qkv_bias, groups=dim)
    ...
    def forward(self, x, last):
        ...
        n, B, C = tsab_in.shape
        q = self.q(tsab_in.permute(0, 2, 1)).permute(0, 1, 2).contiguous()
        kv = self.kv(last_tsab_in.permute(0, 2, 1)).permute(0, 1, 2).contiguous()
...
class NetVideo_base_noStageInteraction_deepcache_action_ffnlw_single_multireuse_multimain_replace_new_normalunfolding_cross_20(
    torch.nn.Module):
    def __init__(self, opt):
        super(
            NetVideo_base_noStageInteraction_deepcache_action_ffnlw_single_multireuse_multimain_replace_new_normalunfolding_cross_20,
            self).__init__()
