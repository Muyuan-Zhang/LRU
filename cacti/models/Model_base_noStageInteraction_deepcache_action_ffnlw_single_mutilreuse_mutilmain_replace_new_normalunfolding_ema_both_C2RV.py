...
class FA_inter(nn.Module):
    ...
    def cal_attention(self, x, last):
        ...
        q = self.to_q_cross(out)
        k = self.to_k_cross(last)
        v = self.to_v_cross(last)
        q, k = map(lambda t: rearrange(t, 'b n B (h d) -> b h B n d', h=self.heads_qk), (q, k))
        v = rearrange(v, 'b n B (h d) -> b h B n d', h=self.heads_v)
        attn = self.cal_atten(q, k)
        out = einsum('b h B i j, b h B j d -> b h B i d', attn, v)
        out = rearrange(out, 'b h B n d -> b n B (h d)')
        out = self.to_out_cross(out)
        return out
...
class TimesAttention3D_inter(nn.Module):
    def __init__(self, dim, num_head, qkv_bias=False, qk_scale=None):
        ...
        self.kv_cross = nn.Linear(dim, dim * 2, bias=qkv_bias)
    ...
    def forward(self, x, last):
        ...
        n, B, C = tsab_in.shape
        q = self.q_self(tsab_in)
        kv = self.kv_self(tsab_in)
...
class NetVideo_base_noStageInteraction_deepcache_action_ffnlw_single_multireuse_multimain_replace_new_normalunfolding_ema_both_C2RV(
    torch.nn.Module):
    def __init__(self, opt):
        super(
            NetVideo_base_noStageInteraction_deepcache_action_ffnlw_single_multireuse_multimain_replace_new_normalunfolding_ema_both_C2RV,
            self).__init__()
