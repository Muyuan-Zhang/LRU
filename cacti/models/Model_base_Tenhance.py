...
class TSAB(nn.Module):
    def __init__(self, dim, num_head, shift=False):
        super().__init__()
        self.tsab = PreNorm(dim, TimesAttention3D(dim, num_head, shift=shift), norm_type='ln')
        self.norm2 = nn.LayerNorm(dim)
...
class FAB_TSAB(nn.Module):
    def __init__(self, dim, sq_dim, window_size=(8, 8), dim_head=28, mult=4, shift=False):
        ...
        self.TSAB = TSAB(dim, num_head=dim_head, shift=shift)
...
class TimesAttention3D(nn.Module):
    def __init__(self, dim, num_head, window_size=(2, 2, 8), qkv_bias=False, qk_scale=None, shift=False):
        super().__init__()
        self.dim = dim
        head_dim = num_head
    ...
    def forward(self, x):
        # if self.shift:
        #     x = x.roll(shifts=1, dims=3).roll(shifts=1, dims=4)
        ...
        x = rearrange(x, "(b h w) (b0 b1 B) c->b B (h b0) (w b1) c",
                      h=h // w_size[0], w=w // w_size[1], b0=w_size[0], b1=w_size[1])
        x = self.proj(x)
        x = x.permute(0, 4, 1, 2, 3)
        # if self.shift:
        #     x = x.roll(shifts=-1, dims=3).roll(shifts=-1, dims=4)
...
class NetVideo_base_Tenhance(torch.nn.Module):
    def __init__(self, opt):
        super(NetVideo_base_Tenhance, self).__init__()
