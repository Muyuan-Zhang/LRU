...
class lightweight_module(torch.nn.Module):
    def __init__(self, in_channels, hidden_dim, is_train):
        ...
        # 多尺度深度卷积
        self.dwconv_k1 = nn.Conv3d(hidden_dim, hidden_dim, kernel_size=1, stride=1, padding=0, groups=hidden_dim,
                                   bias=False)
        self.dwconv_k3 = nn.Conv3d(hidden_dim, hidden_dim, kernel_size=(1, 3, 3), stride=1, padding=(0, 1, 1),
                                   groups=hidden_dim, bias=False)
        self.dwconv_k5 = nn.Conv3d(hidden_dim, hidden_dim, kernel_size=(1, 5, 5), stride=1, padding=(0, 2, 2),
                                   groups=hidden_dim, bias=False)
...
class NetVideo_base_resnet_repffn(torch.nn.Module):
    def __init__(self, opt):
        super(NetVideo_base_resnet_repffn, self).__init__()
