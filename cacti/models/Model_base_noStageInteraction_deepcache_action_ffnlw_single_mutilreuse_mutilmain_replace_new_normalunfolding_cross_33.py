...
# 把SF和cross和fusion融合成一个模块 base 31
# 加一个fusion到前一个SF前面 32 30.59
# 将空间和通道分开 33 30.41
#################
...
class FAB_TSAB_inter(nn.Module):
    def __init__(self, dim, sq_dim, window_size=(8, 8), dim_head=28, mult=4, shift=False):
        ...
        self.fusion1 = nn.Conv3d(dim * 2, dim, kernel_size=1, stride=1, padding=0, bias=False)
        self.fusion2 = nn.Conv3d(dim * 2, dim, kernel_size=1, stride=1, padding=0, bias=False)
        # self.TSAB_inter = TSAB(dim, num_head=dim_head)
        # self.TSAB2 = TSAB(dim, num_head=dim_head)

    def forward(self, x, enc, dec):
        x = self.FAB_inter(x, enc)
        x = x + self.fusion1(torch.cat([enc, x], dim=1))
        x1 = self.SF1(x, enc, enc)
        enc = x1
        x1 = self.TSAB_inter(x1, dec)
        x = x1 + self.fusion2(torch.cat([dec, x1], dim=1))
...
class NetVideo_base_noStageInteraction_deepcache_action_ffnlw_single_multireuse_multimain_replace_new_normalunfolding_cross_33(
    torch.nn.Module):
    def __init__(self, opt):
        super(
            NetVideo_base_noStageInteraction_deepcache_action_ffnlw_single_multireuse_multimain_replace_new_normalunfolding_cross_33,
            self).__init__()
