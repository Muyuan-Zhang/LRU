...
class FA_inter(nn.Module):
    def __init__(self, dim, window_size=(8, 8), dim_head=28, sq_dim=None, shift=True):
        ...
        self.beta = nn.Parameter(torch.zeros((1, dim, 1, 1, 1)), requires_grad=True)
        self.gamma = nn.Parameter(torch.zeros((1, dim, 1, 1, 1)), requires_grad=True)
    ...
    def cal_attention(self, x, last):
        ...
        return out_x + out_last * self.beta, out_last + out_x * self.gamma
...
class IPB_without(nn.Module):
    def __init__(self, in_dim=8, out_dim=4, dim=4, is_train=True):
        ...
        # self.conv_in = nn.Conv3d(in_dim, dim, 3, 1, 1, bias=False)
        # self.conv_in = nn.Conv3d(in_dim, dim, 1, 1, 0, bias=False)
        self.down1 = FAB_TSAB_inter(dim=dim, sq_dim=dim, dim_head=dim, mult=4)
        # # self.downsample1 = nn.Conv2d(dim, dim * 2, 4, 2, 1, bias=False)
        # # 改动
        # # self.downsample1 = nn.Conv3d(dim, dim * 2, 4, 2, 1, bias=False)
        # # 改变下采样，帧数保持不变
...
class TimesAttention3D_inter(nn.Module):
    def __init__(self, dim, num_head, qkv_bias=False, qk_scale=None):
        ...
        self.softmax = nn.Softmax(dim=-1)
        self.inter = StageInteraction(dim)
    ...
    def forward(self, x, last):
        ...
        out_x = out_x.permute(0, 4, 1, 2, 3)
        out_last = out_last.permute(0, 4, 1, 2, 3)
        x = self.inter(out_x, out_last, out_last)
        return x
...
class NetVideo_base_noStageInteraction_deepcache_action_ffnlw_single_multireuse_multimain_replace_new_normalunfolding_ema_both_DBCA_d(
    torch.nn.Module):
    def __init__(self, opt):
        super(
            NetVideo_base_noStageInteraction_deepcache_action_ffnlw_single_multireuse_multimain_replace_new_normalunfolding_ema_both_DBCA_d,
            self).__init__()
