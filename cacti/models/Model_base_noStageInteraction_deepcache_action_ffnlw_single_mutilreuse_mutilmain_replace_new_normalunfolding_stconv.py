...
class STConv3d(nn.Module):
    def __init__(self, dim, groups=1, bias=False):
        super().__init__()
        self.sw_conv = nn.Sequential(
            nn.Conv3d(dim, dim, (1, 3, 3), 1, (0, 1, 1), groups=groups, bias=bias),
            # nn.Conv3d(dim, dim * 4, 1, 1, 0, groups=1, bias=bias),
            nn.LeakyReLU(inplace=True),
            nn.Conv3d(dim, dim, (1, 3, 3), 1, (0, 1, 1), groups=groups, bias=bias),
            # nn.LeakyReLU(inplace=True),
            # nn.Conv3d(dim, dim, 1, 1, 0, groups=1, bias=bias),
            # nn.Conv3d(dim, dim, (1, 3, 3), 1, (0, 1, 1), groups=groups, bias=bias),
            # nn.ReLU(inplace=True),
        )
        self.tw_conv = nn.Sequential(
            nn.Conv3d(dim, dim, (3, 1, 1), 1, (1, 0, 0), groups=groups, bias=bias),
            # nn.Conv3d(dim, dim * 4, 1, 1, 0, groups=1, bias=bias),
            # nn.LeakyReLU(inplace=True),
            # nn.Conv3d(dim, dim, (3, 1, 1), 1, (1, 0, 0), groups=groups, bias=bias),
            # nn.LeakyReLU(inplace=True),
...
class NetVideo_base_noStageInteraction_deepcache_action_ffnlw_single_multireuse_multimain_replace_new_normalunfolding_stconv(
    torch.nn.Module):
    def __init__(self, opt):
        super(
            NetVideo_base_noStageInteraction_deepcache_action_ffnlw_single_multireuse_multimain_replace_new_normalunfolding_stconv,
            self).__init__()
