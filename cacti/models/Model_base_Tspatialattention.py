...
class TimesAttention3D(nn.Module):
    def __init__(self, dim, num_head, qkv_bias=False, qk_scale=None):
        ...
        #     nn.Conv3d(dim, dim, kernel_size=1, stride=1, padding=0),
        #     nn.GELU(),
        # )
        self.q_spatial = nn.Conv3d(dim, dim, 1, 1, 0, bias=qkv_bias)
        self.k_spatial = nn.Conv3d(dim, dim * 4, kernel_size=(1, 3, 3), stride=1, padding=(0, 1, 1), bias=qkv_bias,
                                   groups=dim)
        self.v_spatial = nn.Conv3d(dim, dim * 4, kernel_size=(1, 3, 3), stride=1, padding=(0, 1, 1), bias=qkv_bias,
                                   groups=dim)
    ...
    def forward(self, x):
        ...
        q_spatial = rearrange(q_spatial, "b (h d m) B a w->b a w B h m d", h=self.num_heads, m=1)
        k_spatial, v_spatial = map(lambda t: rearrange(t, 'b (h d m) B a w -> b a w B h m d',
                                                       h=self.num_heads, m=self.M_size[0] * self.M_size[1]),
                                   (k_spatial, v_spatial))
        ...
        spatial_out = self.proj_spa(spatial_out)
...
class NetVideo_base_Tspatialattention(torch.nn.Module):
    def __init__(self, opt):
        super(NetVideo_base_Tspatialattention, self).__init__()
