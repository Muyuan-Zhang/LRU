...
class FA_inter(nn.Module):
    def __init__(self, dim, window_size=(8, 8), dim_head=28, sq_dim=None, shift=True):
        ...
        self.to_v = nn.Linear(dim, dim, bias=False)
        self.to_q = nn.Linear(dim*2, dim*2, bias=False)
        self.to_k = nn.Linear(dim*2, dim*2, bias=False)
        self.to_out = nn.Linear(dim, dim)
...
class FAB_inter(nn.Module):
    def __init__(self, dim, sq_dim, window_size=(8, 8), dim_head=28, mult=4, shift=False):
        ...
        self.norm1 = nn.LayerNorm(dim)
        self.norm1_last = nn.LayerNorm(dim*2)
...
class TimesAttention3D_inter(nn.Module):
    def __init__(self, dim, num_head, qkv_bias=False, qk_scale=None):
        ...
        self.k = nn.Linear(dim*2, dim*2, bias=qkv_bias)
    ...
    def forward(self, x, last):
        ...
        n, B, C = tsab_in.shape
        q = self.q(last_tsab_in)
        k = self.k(last_tsab_in)
        v = self.v(tsab_in)
        q = q.reshape(n, B, self.num_heads, C*2 // self.num_heads).permute(0, 2, 1, 3)
        k = k.reshape(n, B, self.num_heads, C*2 // self.num_heads).permute(0, 2, 1, 3)
...
class IPB_without(nn.Module):
    ...
    def forward(self, x, stage_ins=None, skip=False, stage_outs=None):
        ...
        if not skip:
            ...
            F_reuse = torch.cat((stage_ins[0], stage_ins[1]), dim=1)
            # x1 = self.downReuse1(x, stage_ins[0])
            x = self.down1_cross(x, F_reuse)
            x1 = self.down1(x)
            x1 = self.down1_action(x1, stage_ins[0], stage_ins[1])
            stage_outs.append(x1)
...
class NetVideo_base_noStageInteraction_deepcache_action_ffnlw_single_multireuse_multimain_replace_new_normalunfolding_cross_22(
    torch.nn.Module):
    def __init__(self, opt):
        super(
            NetVideo_base_noStageInteraction_deepcache_action_ffnlw_single_multireuse_multimain_replace_new_normalunfolding_cross_22,
            self).__init__()
