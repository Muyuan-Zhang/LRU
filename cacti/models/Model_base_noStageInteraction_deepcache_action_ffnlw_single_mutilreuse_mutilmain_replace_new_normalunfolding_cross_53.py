...
#################
# 在51基础上接着改
# fusionblock改成通道注意力，增加一个sigmoid相乘
# 52基础上，将主阶段的特征作为指导进行迭代(没有意义)
#################
...
class CrossFFN(nn.Module):
    def __init__(self, dim, mult=1):
        ...
        self.x = nn.Sequential(
            nn.Conv3d(dim, dim * mult, 1, 1, bias=False),
            nn.Conv3d(dim * mult, dim, 3, 1, 1, bias=False, groups=dim),
            GELU()
        )
    ...
    def forward(self, x, last):
        ...
        out = self.x(x_) * self.last(last_) + x
        return out
...
class IPB(nn.Module):
    ...
    def forward(self, x, skip=False, stage_outs=None):
        """
            x: [b,c,B,h,w]
...
class FAB_inter(nn.Module):
    def __init__(self, dim, sq_dim, window_size=(8, 8), dim_head=28, mult=4, shift=False):
        ...
        # self.mpmlp = MPMLP(dim=dim, multi=mult)
        # self.crossffn = CrossFFN(dim=dim)
        # self.res = nn.Conv3d(dim, dim, kernel_size=1, bias=False)
        # self.act_fn = nn.LeakyReLU()
    ...
    def forward(self, x, last):
        ...
        # x_ = self.norm2(x.permute(0, 2, 3, 4, 1)).permute(0, 4, 1, 2, 3)
        # last_ = self.norm2_last(last.permute(0, 2, 3, 4, 1)).permute(0, 4, 1, 2, 3)
        # x = x * torch.sigmoid(self.act_fn(self.res(last))) + self.crossffn(x_, last_)
        # x_ = self.norm2(x)
        # x = self.ffn(x_) + x
        # x = x.permute(0, 4, 1, 2, 3)
        return x
...
class FAB_TSAB_inter(nn.Module):
    def __init__(self, dim, sq_dim, window_size=(8, 8), dim_head=28, mult=4, shift=False):
        ...
        # self.fusion1 = nn.Conv3d(dim * 3, dim, kernel_size=1, stride=1, padding=0)
        # self.fusion2 = nn.Conv3d(dim * 2, dim, kernel_size=1, stride=1, padding=0)
        # self.TSAB_inter = TSAB(dim, num_head=dim_head)
        # self.TSAB2 = TSAB(dim, num_head=dim_head)
        self.crossffn = CrossFFN(dim)

    def forward(self, x, dec):
        # x = x + self.fusion1(torch.cat([x, enc, dec], dim=1))
        x = self.FAB_inter(x, dec)
        # x = x + self.fusion2(torch.cat([x, dec], dim=1))
        x = self.TSAB_inter(x, dec)
        x = self.crossffn(x, dec)
        dec = x
        # x = self.TSAB2(x)
        return x, dec
...
class TimesAttention3D_inter(nn.Module):
    def __init__(self, dim, num_head, qkv_bias=False, qk_scale=None):
        ...
        # self.skip = nn.Conv3d(dim, dim, kernel_size=1, bias=qkv_bias)
        # self.phi = nn.Conv3d(dim, dim, kernel_size=3, padding=1, bias=qkv_bias, groups=dim)
        # self.gamma = nn.Conv3d(dim, dim, kernel_size=3, padding=1, bias=qkv_bias, groups=dim)
        # self.act_fn = nn.LeakyReLU()
...
class TSAB_inter(nn.Module):
    def __init__(self, dim, num_head):
        ...
        # self.crossffn = CrossFFN(dim)
        # self.res = nn.Conv3d(dim, dim, kernel_size=1, stride=1, padding=0, bias=False)
        # self.act_fn = nn.LeakyReLU()
        # self.phi = nn.Conv3d(dim, dim, kernel_size=(3, 3, 3), stride=1, padding=(1, 1, 1), bias=False, groups=dim)
        # self.gamma = nn.Conv3d(dim, dim, kernel_size=(3, 3, 3), stride=1, padding=(1, 1, 1), bias=False, groups=dim)
        # self.act_fn = nn.LeakyReLU()
        # self.norm2 = nn.LayerNorm(dim)
        # self.mpmlp = MPMLP(dim=dim, multi=4)
        # self.ffn = FeedForward(dim=dim)
    ...
    def forward(self, x, last):
        x_ = self.norm1(x.permute(0, 2, 3, 4, 1)).permute(0, 4, 1, 2, 3)
        last_ = self.norm1_last(last.permute(0, 2, 3, 4, 1)).permute(0, 4, 1, 2, 3)
        x = self.tsab_inter(x_, last_) + x
        # x_ = self.norm2(x.permute(0, 2, 3, 4, 1)).permute(0, 4, 1, 2, 3)
        # last_ = self.norm2_last(last.permute(0, 2, 3, 4, 1)).permute(0, 4, 1, 2, 3)
        # x = self.crossffn(x_, last_) + x * torch.sigmoid(self.act_fn(self.res(last)))
        # x = x.permute(0, 2, 3, 4, 1)  # 把通道放在后面
        # x = self.ffn(self.norm2(x)) + x
        # x = x.permute(0, 4, 1, 2, 3)
        return x
...
class ChanelAttention(nn.Module):
    def __init__(self, in_dim, last_dim, dim):
        super().__init__()
        self.dconv = nn.Conv3d(in_dim, in_dim, kernel_size=3, stride=1, padding=1, groups=dim, bias=False)
        self.ln = nn.LayerNorm(in_dim)
        self.conv = nn.Sequential(
            nn.Conv3d(in_dim, in_dim * 2, kernel_size=1, stride=1, padding=0, bias=False),
            nn.GELU(),
            nn.Conv3d(in_dim * 2, dim, kernel_size=1, stride=1, padding=0, bias=False),
        )
        self.CA = nn.Sequential(
            # nn.AdaptiveAvgPool3d(1),
            nn.Conv3d(last_dim, last_dim // 8, kernel_size=1, bias=False),
            nn.GELU(),
            nn.Conv3d(last_dim // 8, dim, kernel_size=1, bias=False),
            nn.Sigmoid(),
        )
...
class FusionBlock(nn.Module):
    def __init__(self, in_dim, out_dim):
        ...
        self.CA = ChanelAttention(in_dim * 2, out_dim, in_dim)

    def forward(self, x, last, feature=None):
...
class IPB_without(nn.Module):
    def __init__(self, in_dim=8, out_dim=4, dim=4, is_train=True):
        ...
        # self.down1_action = StageInteraction(dim=dim)
        self.fusion_block = FusionBlock(in_dim=in_dim, out_dim=dim)
    ...
    def forward(self, x, stage_ins=None, feature=None, skip=False, stage_outs=None):
        """
            x: [b,c,B,h,w]
            return out:[b,c,B,h,w]
        """
        ...
        if True:
            ...
            x = self.fusion_block(x, stage_ins)
            x, dec = self.down1_cross(x, stage_ins)
            # x1 = self.down1_action(x1, stage_ins[0], stage_ins[0])
            # stage_outs.append(dec)
...
class NetVideo_base_noStageInteraction_deepcache_action_ffnlw_single_multireuse_multimain_replace_new_normalunfolding_cross_53(
    torch.nn.Module):
    def __init__(self, opt):
        super(
            NetVideo_base_noStageInteraction_deepcache_action_ffnlw_single_multireuse_multimain_replace_new_normalunfolding_cross_53,
            self).__init__()
        ...
        #     FB(in_dim=opt.dim * 2, dim=opt.dim)
        # ]))
        self.fusion = ChanelAttention(in_dim=opt.dim, last_dim=opt.dim, dim=opt.dim)
        # 改动：添加一个特征提取块
    ...
    def forward(self, g, input_mask=None):
        ...
        for i in range(len(self.reuse)):
            ...
            if j != 0:
                ...
                # if i == 1:
                #     # feature = self.fusion(stage_outputs[1], torch.cat([stage_outputs[0], stage_outputs[1]], dim=1))
                #     stage_output = self.fusion(stage_output, stage_output)
