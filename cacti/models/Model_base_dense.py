...
class ResDNetBlock(nn.Module):
    def __init__(self, in_dim, out_dim, dim, group_num=2):
        super().__init__()
        self.in_dim = in_dim
        self.out_dim = out_dim
        self.IPB_list = nn.ModuleList()
        self.group_num = group_num
        in_dim = in_dim // group_num
        out_dim = out_dim // group_num
        self.dense_conv = nn.ModuleList()
        for i in range(group_num):
            self.IPB_list.append(IPB(in_dim, out_dim, out_dim))
            if i > 0:
                self.dense_conv.append(
                    nn.Sequential(
                        nn.Conv3d(out_dim * (i + 1), out_dim, 1),
                        nn.LeakyReLU(inplace=True)
                    )
                )
        self.last_conv = nn.Conv3d(dim, dim, 1)

    def chunk_input(self, input_list, group_num):
        # 创建一个新的 list，用于存储每个元素的 chunk 结果
        chunked_list = []

        for tensor in input_list:
            # 对 tensor 进行 chunk 操作，按照 group_num 分割通道维度
            chunked_tensor = torch.chunk(tensor, chunks=group_num, dim=1)
            chunked_list.append(chunked_tensor)
        return chunked_list
    ...
    def forward(self, x, stage_ins=None):
        ...
        if stage_ins is not None:
            stage_input_list = self.chunk_input(stage_ins, self.group_num)
            stage_in = stage_input_list[0]
        IPB_out, stage_out = self.IPB_list[0](IPB_in, stage_in)
        out_list.append(IPB_out)
        stage_out_list.append(stage_out)
        for i in range(1, self.group_num):
            in_list = out_list.copy()
            in_list.append(input_list[i])
            if stage_ins is not None:
                stage_in_list = stage_out_list.copy()
                stage_out_list.append(stage_input_list[i])
                stage_in = torch.cat(stage_in_list, dim=1)
            IPB_in = torch.cat(in_list, dim=1)
            IPB_in = self.dense_conv[i - 1](IPB_in)
            IPB_out = self.IPB_list[i](IPB_in)
            out_list.append(IPB_out)
        out = torch.cat(out_list, dim=1)
        out = self.last_conv(out)
        out = x + out
        return out, stage_out
...
class NetVideo_base_dense(torch.nn.Module):
    def __init__(self, opt):
        super(NetVideo_base_dense, self).__init__()
        ...
        self.net_stage_head = nn.ModuleList(nn.ModuleList([
            ResDNetBlock(in_dim=opt.dim * 2, out_dim=opt.dim, dim=opt.dim),
