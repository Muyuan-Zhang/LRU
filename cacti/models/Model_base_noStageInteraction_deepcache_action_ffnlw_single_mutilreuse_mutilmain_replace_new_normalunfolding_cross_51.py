...
class FusionBlock(nn.Module):
    def __init__(self, in_dim, out_dim):
        super().__init__()
        self.fusion = nn.Conv3d(in_dim * 2, out_dim, kernel_size=1, stride=1, padding=0, bias=False)
        self.SF = StageInteraction(dim=in_dim)
        self.CA = ChanelAttention(in_dim)
...
class IPB_without(nn.Module):
    ...
    def forward(self, x, stage_ins=None, skip=False, stage_outs=None):
        ...
        if not skip:
            ...
            x, dec = self.down1_cross(x, stage_ins[0])
...
class NetVideo_base_noStageInteraction_deepcache_action_ffnlw_single_multireuse_multimain_replace_new_normalunfolding_cross_51(
    torch.nn.Module):
    def __init__(self, opt):
        super(
            NetVideo_base_noStageInteraction_deepcache_action_ffnlw_single_multireuse_multimain_replace_new_normalunfolding_cross_51,
            self).__init__()
    ...
    def forward(self, g, input_mask=None):
        ...
        for i in range(len(self.reuse)):
            ...
            # r = self.net_stage_body[i][1   ](to rch.cat([z_ori - y / mu - f, f], dim=1), Phi, Phi_compressive)
