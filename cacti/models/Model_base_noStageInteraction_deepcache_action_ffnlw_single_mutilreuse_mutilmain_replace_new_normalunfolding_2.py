...
class SCAB(nn.Module):
    def __init__(self, dim, sq_dim, window_size=(8, 8), dim_head=28, mult=4, shift=False):
        ...
        # self.fusion = FusionBlock(in_dim=dim, out_dim=dim)
...
class DSTCT(nn.Module):
    def __init__(self, in_dim=8, out_dim=4, dim=4):
        ...
        # self.down1 = STSAB(dim=dim, sq_dim=dim, dim_head=dim, mult=4)
        self.down1 = STCAB(dim=dim, sq_dim=dim, dim_head=dim, mult=4)
        self.fusion1 = nn.Conv3d(dim * 2, dim, 1, 1, 0, bias=False)
        # self.reuse = STCAB(dim=dim, sq_dim=dim, dim_head=dim, mult=2)
...
class NetVideo_base_noStageInteraction_deepcache_action_ffnlw_single_multireuse_multimain_replace_new_normalunfolding_2(
    torch.nn.Module):
    def __init__(self, opt):
        super(
            NetVideo_base_noStageInteraction_deepcache_action_ffnlw_single_multireuse_multimain_replace_new_normalunfolding_2,
            self).__init__()
