...
class IPB_without(nn.Module):
    ...
    def forward(self, x, stage_ins=None, skip=False, stage_outs=None):
        ...
        if not skip:
            ...
            x = self.down1(x)
            x = self.down1_action(x, stage_ins[0], stage_ins[1])
            x1 = self.downReuse1(x, stage_ins[0])
            stage_outs.append(x1)
            # x1 = self.down1(x)
            x = x1 + self.fusion1(torch.cat([stage_ins[1], x1], dim=1))
...
class NetVideo_base_noStageInteraction_deepcache_action_ffnlw_single_multireuse_multimain(torch.nn.Module):
    def __init__(self, opt):
        super(NetVideo_base_noStageInteraction_deepcache_action_ffnlw_single_multireuse_multimain, self).__init__()
