...
class IPB_without(nn.Module):
    ...
    def forward(self, x, stage_ins=None, skip=False, stage_outs=None):
        ...
        if not skip:
            ...
            x1 = self.down1(x)
            # x1 = self.downReuse1(x, stage_ins[0])
            x2 = self.fusion_block(x, stage_ins[0])
            x2, dec = self.down1_cross(x2, stage_ins[0])
            x = x1 + x2
...
class NetVideo_base_noStageInteraction_deepcache_action_ffnlw_single_multireuse_multimain_replace_new_normalunfolding_cross_50(
    torch.nn.Module):
    def __init__(self, opt):
        super(
            NetVideo_base_noStageInteraction_deepcache_action_ffnlw_single_multireuse_multimain_replace_new_normalunfolding_cross_50,
            self).__init__()
