...
class FAB_inter(nn.Module):
    def __init__(self, dim, sq_dim, window_size=(8, 8), dim_head=28, mult=4, shift=False):
        ...
        self.fa_inter = FA_inter(dim=dim, window_size=window_size, dim_head=dim_head, sq_dim=sq_dim, shift=shift)
        self.norm2 = nn.LayerNorm(dim)
        # self.mpmlp = MPMLP(dim=dim, multi=mult)
        self.ffn = FeedForward(dim=dim, mult=mult)
        ...
        self.fa = FA(dim=dim, window_size=window_size, dim_head=dim_head, sq_dim=sq_dim, shift=shift)
        self.ffn2 = FeedForward(dim=dim, mult=mult)
...
class TSAB_inter(nn.Module):
    def __init__(self, dim, num_head):
        ...
        self.norm1_last = nn.LayerNorm(dim)
        self.norm2 = nn.LayerNorm(dim)
        self.tsab = TimesAttention3D(dim, num_head)
        self.norm3 = nn.LayerNorm(dim)
        self.norm4 = nn.LayerNorm(dim)
        # self.mpmlp = MPMLP(dim=dim, multi=4)
        self.ffn = FeedForward(dim=dim)
        self.ffn2 = FeedForward(dim=dim)
    ...
    def forward(self, x, last):
        ...
        x_ = self.norm3(x).permute(0, 4, 1, 2, 3)
        x = x.permute(0, 4, 1, 2, 3)
        x = self.tsab(x_) + x
        x = x.permute(0, 2, 3, 4, 1)  # 把通道放在后面
...
class NetVideo_base_noStageInteraction_deepcache_action_ffnlw_single_multireuse_multimain_replace_new_normalunfolding_ema_both_C2RV_divide(
    torch.nn.Module):
    def __init__(self, opt):
        super(
            NetVideo_base_noStageInteraction_deepcache_action_ffnlw_single_multireuse_multimain_replace_new_normalunfolding_ema_both_C2RV_divide,
            self).__init__()
