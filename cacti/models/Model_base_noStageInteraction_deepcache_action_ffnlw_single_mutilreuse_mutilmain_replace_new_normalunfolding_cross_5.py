...
class FA_inter(nn.Module):
    ...
    def cal_attention(self, x, enc, dec):
        q = self.to_q(x)
        k = self.to_k(enc)
        v = self.to_v(dec)
        q, k = map(lambda t: rearrange(t, 'b n B (h d) -> b h B n d', h=self.heads_qk), (q, k))
    ...
    def forward(self, x, enc, dec):
        ...
        x_inp = rearrange(x, 'b B (h b0) (w b1) c -> (b h w) (b0 b1) B c', b0=w_size[0], b1=w_size[1])
        enc = rearrange(enc, 'b B (h b0) (w b1) c -> (b h w) (b0 b1) B c', b0=w_size[0], b1=w_size[1])
        dec = rearrange(dec, 'b B (h b0) (w b1) c -> (b h w) (b0 b1) B c', b0=w_size[0], b1=w_size[1])
        out = self.cal_attention(x_inp, enc, dec)
        out = rearrange(out, '(b h w) (b0 b1) B c -> b B (h b0) (w b1) c', h=h // w_size[0], w=w // w_size[1],
                        b0=w_size[0])
        if self.shift:
            out = out.roll(shifts=-4, dims=2).roll(shifts=-4, dims=3)
...
class FAB_inter(nn.Module):
    def __init__(self, dim, sq_dim, window_size=(8, 8), dim_head=28, mult=4, shift=False):
        ...
        self.norm1 = nn.LayerNorm(dim)
        self.norm1_dec = nn.LayerNorm(dim)
        self.norm1_enc = nn.LayerNorm(dim)
    ...
    def forward(self, x, enc, dec):
        ...
        # x = x.permute(0, 2, 3, 1)  # 把通道放在后面
        x = x.permute(0, 2, 3, 4, 1)  # 把通道放在后面
        dec = dec.permute(0, 2, 3, 4, 1)  # 把通道放在后面
        enc = enc.permute(0, 2, 3, 4, 1)  # 把通道放在后面
        x_ = self.norm1(x)
        dec_ = self.norm1_dec(dec)
        enc_ = self.norm1_enc(enc)
        x = self.fa(x_, enc_, dec_) + x
        # x_ = self.norm2(x)
        # x = self.ffn(x_) + x
        x = x.permute(0, 4, 1, 2, 3)
        return x
...
class FAB_TSAB_inter(nn.Module):
    ...
    def forward(self, x, enc, dec):
        x = self.FAB_inter(x, enc, dec)
        x = self.TSAB_inter(x, enc, dec)
        # x = self.TSAB2(x)
        return x
...
class TimesAttention3D_inter(nn.Module):
    ...
    def forward(self, x, enc, dec):
        ...
        enc = rearrange(enc, "b c B h w->(b h w) B c")
        dec = rearrange(dec, "b c B h w->(b h w) B c")
        n, B, C = tsab_in.shape
        q = self.q(tsab_in)
        k = self.k(enc)
        v = self.v(dec)
...
class TSAB_inter(nn.Module):
    ...
    def forward(self, x, enc, dec):
        ...
        enc = self.norm1_last(enc.permute(0, 2, 3, 4, 1)).permute(0, 4, 1, 2, 3)
        dec = self.norm1_last(dec.permute(0, 2, 3, 4, 1)).permute(0, 4, 1, 2, 3)
        x = self.tsab_inter(x_, enc, dec) + x
        # x = x.permute(0, 2, 3, 4, 1)  # 把通道放在后面
        # x = self.ffn(self.norm2(x)) + x
        # x = x.permute(0, 4, 1, 2, 3)
        return x
...
class IPB_without(nn.Module):
    ...
    def forward(self, x, stage_ins=None, skip=False, stage_outs=None):
        ...
        if not skip:
            ...
            x1 = self.down1(x)
            # x1 = self.downReuse1(x, stage_ins[0])
            x1 = self.down1_cross(x1, stage_ins[0], stage_ins[1])
            x1 = self.down1_action(x1, stage_ins[0], stage_ins[1])
            stage_outs.append(x1)
...
class NetVideo_base_noStageInteraction_deepcache_action_ffnlw_single_multireuse_multimain_replace_new_normalunfolding_cross_5(
    torch.nn.Module):
    def __init__(self, opt):
        super(
            NetVideo_base_noStageInteraction_deepcache_action_ffnlw_single_multireuse_multimain_replace_new_normalunfolding_cross_5,
            self).__init__()
