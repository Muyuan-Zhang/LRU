...
#         x = enc.permute(0, 2, 3, 4, 1)
#         x = self.ln1(x)
#         x = x.permute(0, 4, 1, 2, 3)
#         x1 = enc + self.branch1(x)
...
class StageInteraction(nn.Module):
    def __init__(self, dim):
        ...
        self.phi = nn.Conv3d(dim, dim, (3, 3, 3), 1, (1, 1, 1), bias=False, groups=dim)
...
class IPB(nn.Module):
    ...
    def forward(self, x, stage_ins=None, skip=False, stage_outs=None):
        ...
        if not skip:
            ...
            x1 = self.downReuse1(stage_ins[0])
            stage_outs.append(self.down1_action(x1, stage_ins[0], stage_ins[5]))
            # x1 = stage_outs[0]

            x2 = self.downReuse2(stage_ins[1])
            stage_outs.append(self.down2_action(x2, stage_ins[1], stage_ins[4]))
            # x2 = stage_outs[1]

            x_local = self.bottleneck_localReuse(stage_ins[2])
            stage_outs.append(self.bottleneck_local_action(x_local, stage_ins[2], stage_ins[3]))

            x_swin = self.bottleneck_swinReuse(stage_ins[2] + stage_ins[3])
            stage_outs.append(self.bottleneck_swin_action(x_swin, stage_ins[3], stage_ins[2]))

            x = torch.cat([stage_outs[2], stage_outs[3]], dim=1)

            x = self.upsample2(x)
            x = x2 + self.reuseFusion2(torch.cat([x, x2, stage_ins[4]], dim=1))
            x = self.upReuse2(x)
            stage_outs.append(self.up2_action(x, stage_ins[4], stage_ins[1]))

            x = self.upsample1(stage_outs[4])
            x = x1 + self.reuseFusion1(torch.cat([x, x1, stage_ins[5]], dim=1))
            x = self.upReuse1(x)
            x = self.up1(x)
            stage_outs.append(self.up1_action(x, stage_ins[5], stage_ins[0]))
...
class NetVideo_base_removeDS(torch.nn.Module):
    def __init__(self, opt):
        super(NetVideo_base_removeDS, self).__init__()
