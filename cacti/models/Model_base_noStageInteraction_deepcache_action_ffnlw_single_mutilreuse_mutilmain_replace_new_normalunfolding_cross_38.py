...
class TSAB_inter(nn.Module):
    def __init__(self, dim, num_head):
        ...
        self.conv1 = nn.Conv3d(dim, dim, kernel_size=1, stride=1, padding=0, bias=False)
    ...
    def forward(self, x, last):
        ...
        last = self.norm2_last(last.permute(0, 2, 3, 4, 1)).permute(0, 4, 1, 2, 3)
        x = self.crossffn(x_, last) + x + self.conv1(last)
        # x = x.permute(0, 2, 3, 4, 1)  # 把通道放在后面
        # x = self.ffn(self.norm2(x)) + x
        # x = x.permute(0, 4, 1, 2, 3)
        return x
...
class IPB_without(nn.Module):
    ...
    def forward(self, x, stage_ins=None, skip=False, stage_outs=None):
        ...
        if not skip:
            ...
            x1 = self.down1(x)
            # x1 = self.downReuse1(x, stage_ins[0])
            x1 = self.down1_cross(x1, stage_ins[0])
...
class NetVideo_base_noStageInteraction_deepcache_action_ffnlw_single_multireuse_multimain_replace_new_normalunfolding_cross_38(
    torch.nn.Module):
    def __init__(self, opt):
        super(
            NetVideo_base_noStageInteraction_deepcache_action_ffnlw_single_multireuse_multimain_replace_new_normalunfolding_cross_38,
            self).__init__()
