...
class NetVideo_conv3d_TSAB_shareBody_action_changeSample_seriesConnection_withoutDP_deepcache_keepy(torch.nn.Module):
    def __init__(self, opt):
        super(NetVideo_conv3d_TSAB_shareBody_action_changeSample_seriesConnection_withoutDP_deepcache_keepy, self).__init__()
    ...
    def forward(self, g, input_mask=None):
        ...
        if not self.body_share_params:
            ...
            for i in range(self.stage - 2):
                ...
                Phi_f = self.mul_Phif(Phi, z - y / mu)
                f = z - y / mu + self.mul_PhiTg(Phi, torch.div(g - Phi_f, mu + PhiPhiT))  # 有个reverse操作需要去掉 cc
