...
# 改用fusion也降低
# 优化SF 残差连接并乘可学习参数
# 优化SF 残差连接并乘可学习参数 加上fusion
################################
# decoder的生成不使用阶段融合结果，而是使用交叉注意力
#########################################
...
class FA_inter(nn.Module):
    def __init__(self, dim, window_size=(8, 8), dim_head=28, sq_dim=None, shift=True):
        ...
        self.to_v_self = nn.Linear(dim, dim, bias=False)
        self.to_q_self = nn.Linear(dim, dim, bias=False)
        # self.to_k_self = nn.Linear(dim, dim, bias=False)

        self.to_v_cross = nn.Linear(dim, dim, bias=False)
        self.to_q_cross = nn.Linear(dim, dim, bias=False)
        # self.to_k_cross = nn.Linear(dim, dim, bias=False)
        self.to_out_self = nn.Linear(dim, dim)
        self.to_out_cross = nn.Linear(dim, dim)
        # self.fusion = nn.Conv3d(dim * 2, dim, 1, 1, 0, bias=False)
        self.beta = nn.Parameter(torch.zeros((1, 1, 1, 1, dim)), requires_grad=True)
        self.gamma = nn.Parameter(torch.zeros((1, 1, 1, 1, dim)), requires_grad=True)

    def cal_attention(self, x, last):
        q_x = self.to_q_self(x)
        v_x = self.to_v_self(x)
        q_last = self.to_q_cross(last)
        v_last = self.to_v_cross(last)
        q_x = torch.nn.functional.normalize(q_x, dim=-1)
        q_last = torch.nn.functional.normalize(q_last, dim=-1)
        ...
        out_x = self.to_out_self(out_x)
        out_last = self.to_out_cross(out_last)
    ...
    def forward(self, x, last):
        ...
        out_x, out_last = self.cal_attention(x_inp, last)
        out_x = rearrange(out_x, '(b h w) (b0 b1) B c -> b B (h b0) (w b1) c', h=h // w_size[0], w=w // w_size[1],
                          b0=w_size[0])
        out_last = rearrange(out_last, '(b h w) (b0 b1) B c -> b B (h b0) (w b1) c', h=h // w_size[0], w=w // w_size[1],
                             b0=w_size[0])

        out_x = out_x + out_x * self.beta
        out_last = out_last + out_last * self.gamma
        # out_last = out_last.permute(0, 4, 1, 2, 3)
        # out_x = (out_x + x).permute(0, 4, 1, 2, 3)
        # out_last = (out_last + x).permute(0, 4, 1, 2, 3)
        out_x = out_x.permute(0, 4, 1, 2, 3)
        out_last = out_last.permute(0, 4, 1, 2, 3)

        if self.shift:
            out_x = out_x.roll(shifts=-4, dims=2).roll(shifts=-4, dims=3)
        return out_x, out_last
...
class FAB_inter(nn.Module):
    ...
    def forward(self, x, last):
        ...
        x = x.permute(0, 4, 1, 2, 3) + x_
        last = last.permute(0, 4, 1, 2, 3) + last_
        # x_ = self.norm2(x)
        # x = self.ffn(x_) + x
        return x, last
...
class IPB_without(nn.Module):
    ...
    def forward(self, x, stage_ins=None, skip=False, stage_outs=None):
        ...
        if not skip:
            ...
            x = self.down1(x)
            # x1 = self.downReuse1(x, stage_ins[0])
            x1 = self.down1_action(x, stage_ins[0], stage_ins[1])
            x, encoder, decoder = self.down1_cross(x, stage_ins[0], stage_ins[1])

            stage_outs.append(encoder)

            x = x1 + self.fusion1(torch.cat([stage_ins[1], x1], dim=1))
            # x = self.up1(x)
            # x = self.upReuse1(x, stage_ins[1])

            x = self.up1_action(x, stage_ins[0], stage_ins[1])
            stage_outs.append(decoder)
...
class TimesAttention3D_inter(nn.Module):
    def __init__(self, dim, num_head, qkv_bias=False, qk_scale=None):
        ...
        self.proj_self = nn.Linear(dim, dim)
        self.proj_cross = nn.Linear(dim, dim)
        self.softmax = nn.Softmax(dim=-1)
        # self.fusion = nn.Conv3d(dim * 2, dim, 1, 1, 0, bias=False)
        # self.SF = StageInteraction(dim)
        self.beta = nn.Parameter(torch.zeros((1, 1, 1, 1, dim)), requires_grad=True)
        self.gamma = nn.Parameter(torch.zeros((1, 1, 1, 1, dim)), requires_grad=True)
    ...
    def forward(self, x, last):
        ...
        n, B, C = tsab_in.shape
        q_x = self.q_self(tsab_in)
        v_x = self.v_self(tsab_in)
        q_last = self.q_cross(last_tsab_in)
        v_last = self.v_cross(last_tsab_in)
        ...
        # 无嵌入向量
        attn = self.softmax(attn)
        out_x = (attn @ v_x).transpose(1, 2).reshape(n, B, C)
        out_last = (attn @ v_last).transpose(1, 2).reshape(n, B, C)
        out_x = self.proj_self(out_x)
        out_last = self.proj_cross(out_last)
        out_x = rearrange(out_x, "(b h w) B c->b B h w c", h=h, w=w)
        out_last = rearrange(out_last, "(b h w) B c->b B h w c", h=h, w=w)
        out_x = out_x + out_x * self.gamma
        out_last = out_last + out_last * self.gamma
        out_x = out_x.permute(0, 4, 1, 2, 3)
        out_last = out_last.permute(0, 4, 1, 2, 3)
        return out_x, out_last
...
class NetVideo_base_noStageInteraction_deepcache_action_ffnlw_single_multireuse_multimain_replace_new_normalunfolding_ema_both_DBCA(
    torch.nn.Module):
