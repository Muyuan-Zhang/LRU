...
#################
# 46 去掉一个基础模块，改用两个cross
# 使用前两个阶段的encoder和decoder
#################
...
class IPB_without(nn.Module):
    ...
    def forward(self, x, stage_ins=None, skip=False, stage_outs=None):
        ...
        if not skip:
            ...
            stage_outs.append(x1)
            x = self.down1(x1)
            x = x + self.fusion1(torch.cat([x, x1], dim=1))
            # x = self.up1(x)
            # x = self.upReuse1(x, stage_ins[1])
            x = self.down2_cross(x, stage_ins[1])
...
class NetVideo_base_noStageInteraction_deepcache_action_ffnlw_single_multireuse_multimain_replace_new_normalunfolding_cross_46(
    torch.nn.Module):
    def __init__(self, opt):
        super(
            NetVideo_base_noStageInteraction_deepcache_action_ffnlw_single_multireuse_multimain_replace_new_normalunfolding_cross_46,
            self).__init__()
