...
#################
# 输出用dw1x1x1，qkv用dwconv3x3x3,qkv之前再经过一个conv1x1x1
#################
...
class FA_inter(nn.Module):
    def __init__(self, dim, window_size=(8, 8), dim_head=28, sq_dim=None, shift=True):
        ...
        self.to_q_3 = nn.Conv3d(dim, dim, 3, 1, padding=1, bias=False, groups=dim)
        self.to_q = nn.Conv3d(dim, dim, 1, 1, padding=0, bias=False)
        self.to_k_3 = nn.Conv3d(dim, dim, 3, 1, padding=1, bias=False, groups=dim)
        self.to_k = nn.Conv3d(dim, dim, 1, 1, padding=0, bias=False)
        self.to_v_3 = nn.Conv3d(dim, dim, 3, 1, padding=1, bias=False, groups=dim)
        self.to_v = nn.Conv3d(dim, dim, 1, 1, padding=0, bias=False)
    ...
    def cal_attention(self, x, last):
        w_size = self.window_size
        q = self.to_q_3(self.to_q(x.permute(0, 4, 1, 2, 3))).permute(0, 2, 3, 4, 1).contiguous()
        k = self.to_k_3(self.to_k(last.permute(0, 4, 1, 2, 3))).permute(0, 2, 3, 4, 1).contiguous()
        v = self.to_v_3(self.to_v(last.permute(0, 4, 1, 2, 3))).permute(0, 2, 3, 4, 1).contiguous()
        q = rearrange(q, 'b B (h b0) (w b1) c -> (b h w) (b0 b1) B c', b0=w_size[0], b1=w_size[1])
        k = rearrange(k, 'b B (h b0) (w b1) c -> (b h w) (b0 b1) B c', b0=w_size[0], b1=w_size[1])
        v = rearrange(v, 'b B (h b0) (w b1) c -> (b h w) (b0 b1) B c', b0=w_size[0], b1=w_size[1])
    ...
    def forward(self, x, last):
        ...
        out = rearrange(out, '(b h w) (b0 b1) B c -> b B (h b0) (w b1) c', h=h // w_size[0], w=w // w_size[1],
                        b0=w_size[0])
        out = self.to_out(out.permute(0, 4, 1, 2, 3)).permute(0, 2, 3, 4, 1)
...
class TimesAttention3D_inter(nn.Module):
    def __init__(self, dim, num_head, qkv_bias=False, qk_scale=None):
        ...
        self.q_3 = nn.Conv3d(dim, dim, kernel_size=3, padding=1, bias=qkv_bias, groups=dim)
        self.q = nn.Conv3d(dim, dim, kernel_size=1, padding=0, bias=qkv_bias)
        self.k_3 = nn.Conv3d(dim, dim, kernel_size=3, padding=1, bias=qkv_bias, groups=dim)
        self.k = nn.Conv3d(dim, dim, kernel_size=1, padding=0, bias=qkv_bias)
        self.v_3 = nn.Conv3d(dim, dim, kernel_size=3, padding=1, bias=qkv_bias, groups=dim)
        self.v = nn.Conv3d(dim, dim, kernel_size=1, padding=0, bias=qkv_bias)
    ...
    def forward(self, x, last):
        ...
        q = self.q_3(self.q(x)).permute(0, 2, 1, 3, 4)
        k = self.k_3(self.k(last)).permute(0, 2, 1, 3, 4)
        v = self.v_3(self.v(last)).permute(0, 2, 1, 3, 4)
        q = rearrange(q, "b B c h w->(b h w) B c")
        k = rearrange(k, "b B c h w->(b h w) B c")
        v = rearrange(v, "b B c h w->(b h w) B c")
        n, B, C = q.shape
        ...
        x = self.proj(x.permute(0, 4, 1, 2, 3))
        # x = x.permute(0, 4, 1, 2, 3)
        return x
...
class IPB_without(nn.Module):
    ...
    def forward(self, x, stage_ins=None, skip=False, stage_outs=None):
        ...
        if not skip:
            ...
            x1 = self.down1(x)
            # x1 = self.downReuse1(x, stage_ins[0])
            x1 = self.down1_cross(x1, stage_ins[1])
            x1 = self.down1_action(x1, stage_ins[0], stage_ins[1])
            stage_outs.append(x1)
...
class NetVideo_base_noStageInteraction_deepcache_action_ffnlw_single_multireuse_multimain_replace_new_normalunfolding_cross_25(
    torch.nn.Module):
    def __init__(self, opt):
        super(
            NetVideo_base_noStageInteraction_deepcache_action_ffnlw_single_multireuse_multimain_replace_new_normalunfolding_cross_25,
            self).__init__()
