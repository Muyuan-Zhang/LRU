...
class FourierUnit(nn.Module):
    def __init__(self, embed_dim, fft_norm='ortho'):
        ...
        self.conv_layer = torch.nn.Conv3d(embed_dim * 2, embed_dim * 2, 1, 1, 0, groups=embed_dim * 2, bias=False)
    ...
    def forward(self, x):
        ...
        ffted = ffted.permute(0, 1, 5, 2, 3, 4).contiguous()  # (batch, c, 2, t, h, w/2+1)
        ffted = ffted.view((batch, -1,) + ffted.size()[3:])

        ffted = self.conv_layer(ffted)  # (batch, c*2, t, h, w/2+1)
        ffted = self.relu(ffted)

        ffted = ffted.view((batch, -1, 2,) + ffted.size()[2:])  # (batch,c, t, h, w/2+1, 2)
...
class SpectralTransform(nn.Module):
    def __init__(self, embed_dim, last_conv=True):
        # bn_layer not used
        super(SpectralTransform, self).__init__()
        self.last_conv = last_conv

        self.conv1 = nn.Sequential(
            nn.Conv3d(embed_dim, embed_dim, 1, 1, 0, groups=embed_dim),
            nn.LeakyReLU(negative_slope=0.2, inplace=True)
        )
        self.fu = FourierUnit(embed_dim)

        self.conv2 = nn.Conv3d(embed_dim, embed_dim, 1, 1, 0, groups=embed_dim)

        if self.last_conv:
            self.last_conv = nn.Conv3d(embed_dim, embed_dim, 3, 1, 1, groups=embed_dim)
...
class ResB(nn.Module):
    def __init__(self, embed_dim, red=1):
        ...
        self.body = nn.Sequential(
            nn.Conv3d(embed_dim, embed_dim // red, (3, 3, 3), 1, (1, 1, 1), groups=embed_dim),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv3d(embed_dim // red, embed_dim, (3, 3, 3), 1, (1, 1, 1), groups=embed_dim),
        )
...
class NetVideo_base_noStageInteraction_deepcache_action_ffnlw_single_multireuse_multimain_replace_new_normalunfolding_fourier(
    torch.nn.Module):
    def __init__(self, opt):
        super(
            NetVideo_base_noStageInteraction_deepcache_action_ffnlw_single_multireuse_multimain_replace_new_normalunfolding_fourier,
            self).__init__()
