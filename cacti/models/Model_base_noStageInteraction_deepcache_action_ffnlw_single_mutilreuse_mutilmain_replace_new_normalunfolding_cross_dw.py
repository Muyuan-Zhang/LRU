...
class FA_inter(nn.Module):
    def __init__(self, dim, window_size=(8, 8), dim_head=28, sq_dim=None, shift=True):
        ...
        self.to_q_dw = nn.Conv3d(dim, dim, (1, 3, 3), 1, (0, 1, 1), bias=False, groups=dim)
        self.to_k_dw = nn.Conv3d(dim, dim, (1, 3, 3), 1, (0, 1, 1), bias=False, groups=dim)
        self.to_v_dw = nn.Conv3d(dim, dim, (1, 3, 3), 1, (0, 1, 1), bias=False, groups=dim)
        self.to_out = nn.Linear(dim, dim)
    ...
    def cal_attention(self, x, last):
        ...
        k = self.to_k(last)
        v = self.to_v(last)
        q = self.to_q_dw(q.permute(0, 4, 1, 2, 3)).permute(0, 2, 3, 4, 1).contiguous()
        k = self.to_k_dw(k.permute(0, 4, 1, 2, 3)).permute(0, 2, 3, 4, 1).contiguous()
        v = self.to_v_dw(v.permute(0, 4, 1, 2, 3)).permute(0, 2, 3, 4, 1).contiguous()
        w_size = self.window_size
...
class TimesAttention3D_inter(nn.Module):
    def __init__(self, dim, num_head, qkv_bias=False, qk_scale=None):
        ...
        self.q_dw = nn.Conv3d(dim, dim, (1, 3, 3), 1, (0, 1, 1), bias=False, groups=dim)

        self.kv = nn.Linear(dim, dim * 2, bias=qkv_bias)
        self.kv_dw = nn.Conv3d(dim * 2, dim * 2, (1, 3, 3), 1, (0, 1, 1), bias=False, groups=dim)
    ...
    def forward(self, x, last):
        x = x.permute(0, 2, 3, 4, 1)
        last = last.permute(0, 2, 3, 4, 1)
        _, _, w, h, _ = x.shape
        q = self.q(x)
        q = self.q_dw(q.permute(0, 4, 1, 2, 3))
        kv = self.kv(last)
        kv = self.kv_dw(kv.permute(0, 4, 1, 2, 3))
        q = rearrange(q, "b c B h w->(b h w) B c")
        kv = rearrange(kv, "b c B h w->(b h w) B c")
        n, B, C = q.shape
...
class NetVideo_base_noStageInteraction_deepcache_action_ffnlw_single_multireuse_multimain_replace_new_normalunfolding_cross_dw(
    torch.nn.Module):
    def __init__(self, opt):
        super(
            NetVideo_base_noStageInteraction_deepcache_action_ffnlw_single_multireuse_multimain_replace_new_normalunfolding_cross_dw,
            self).__init__()
