...
class SCAB(nn.Module):
    def __init__(self, dim, sq_dim, window_size=(8, 8), dim_head=28, mult=4, shift=False):
        ...
        self.fa = CrossPreNorm(dim,
                               ...
                               norm_type='crossln')
        # self.CA = ChanelSpatialAttention(dim, dim)
...
class STCAB(nn.Module):
    ...
    def forward(self, x, spatial, time):
        x = self.SCA(x, spatial)
        x = self.TCA(x, time)
        return x, spatial
...
class DSTCT(nn.Module):
    def __init__(self, in_dim=8, out_dim=4, dim=4):
        ...
        # self.main = STSAB(dim=dim, sq_dim=dim, dim_head=dim//2, mult=4)
        self.reuse = STCAB(dim=dim, sq_dim=dim, dim_head=dim//2, mult=4)
    ...
    def forward(self, x, stage_ins=None, skip=False, stage_outs=None):
        ...
        if stage_outs is None:
            stage_outs = []
        x_in = x
        x, spatial = self.reuse(x, stage_ins[0], stage_ins[1])
        # x = self.fusion_block(x, stage_ins[0])
        # x = self.main(x)
        stage_outs.append(spatial)
        stage_outs.append(x)
        out = x + x_in
        # out = self.conv_out(x) + x_in
...
class NetVideo_base_noStageInteraction_deepcache_action_ffnlw_single_multireuse_multimain_replace_new_normalunfolding_4(
    torch.nn.Module):
