...
class MultiScaleFeatureFusionModule(nn.Module):
    def __init__(self, dim):
        ...
        # 多尺度卷积核，使用空洞卷积增加感受野
        self.conv1 = nn.Conv3d(dim, dim, kernel_size=(1, 3, 3), padding=(0, 1, 1), dilation=1, bias=False)
        self.conv2 = nn.Conv3d(dim, dim, kernel_size=(1, 3, 3), padding=(0, 2, 2), dilation=2, bias=False)
        self.conv3 = nn.Conv3d(dim, dim, kernel_size=(1, 3, 3), padding=(0, 3, 3), dilation=3, bias=False)
        ...
        # 用于融合多尺度特征的1x1卷积
        self.fusion = nn.Conv3d(dim * 3, dim, kernel_size=1, bias=False)

    def forward(self, x):
        # 多尺度特征提取
        x1 = self.conv1(x)  # 普通卷积
        x2 = self.conv2(x)  # 空洞卷积，扩展感受野
        x3 = self.conv3(x)  # 更大感受野的空洞卷积
...
class EdgeEnhanceModule(nn.Module):
    def __init__(self, dim):
        ...
        # 3x3 卷积用于增强边缘特征
        self.conv = nn.Conv3d(dim, dim, kernel_size=(1, 3, 3), padding=(0, 1, 1), bias=False)
        self.ln = nn.LayerNorm(dim)
        self.relu = nn.ReLU(inplace=True)

    def forward(self, x):
        x = self.conv(x)  # 提取特征
        x = self.ln(x.permute(0, 2, 3, 4, 1)).permute(0, 4, 1, 2, 3)  # 归一化
        x = self.relu(x)  # 非线性激活
        return x
...
class MultiScaleFeatureFusion(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        # 使用不同卷积核尺寸的并行卷积
        # self.conv3x3 = nn.Conv3d(in_channels, out_channels, kernel_size=(1, 3, 3), padding=(0, 1, 1))
        self.conv5x5 = nn.Conv3d(in_channels, out_channels, kernel_size=(1, 5, 5), padding=(0, 2, 2))
        self.conv7x7 = nn.Conv3d(in_channels, out_channels, kernel_size=(1, 7, 7), padding=(0, 3, 3))

        # 最终融合后的卷积层，用于整合不同尺度特征
        self.fusion_conv = nn.Conv3d(out_channels * 2, out_channels, kernel_size=1)
    ...
    def forward(self, x):
        ...
        # 将多尺度特征拼接在通道维度
        fused = torch.cat([feat_5x5, feat_7x7], dim=1)
...
class LightweightAdaptationModule(nn.Module):
    def __init__(self, channels, reduction_ratio=16):
        """
        轻量化处理模块：使用1x1卷积进行特征调整，并加入轻量级通道注意力机制。
        Args:
            channels (int): 输入特征的通道数
            reduction_ratio (int): 通道缩减比例，用于SE Block（默认值为16）
        """
        super(LightweightAdaptationModule, self).__init__()

        # 1x1卷积，用于适应缓存特征到当前阶段的需求
        self.conv1x1 = nn.Conv3d(channels, channels, kernel_size=1, bias=False)

        # SE Block 通道注意力，用于轻量级特征加权
...
class F_TSAB(nn.Module):
    def __init__(self, in_dim=8, out_dim=4, dim=4):
        super(F_TSAB, self).__init__()
        ...
        self.theta = Mu_Estimator(in_nc=dim * 4)
    ...
    def forward(self, x, stage_ins=None, skip=False, stage_outs=None):
        ...
        if not skip:
            ...
            x2 = self.multiScaleFeatureFusion(x)
            stage_outs.append(self.down2_action(x2, stage_ins[1], stage_ins[4]))
        x = self.downsample2(stage_outs[1])
        theta = self.theta(x)
        # 通道 * 4
        x_local = self.bottleneck_local(x[:, :c*2, :, :]) * theta
        if stage_ins:
        ...
        x_swin = self.bottleneck_swin(x[:, 2*c:, :, :] + stage_outs[2]) * (1 - theta)
        stage_outs.append(x_swin)
...
class Conv_F_TSAB(nn.Module):
    def __init__(self, in_dim=8, out_dim=4, dim=4):
        super(Conv_F_TSAB, self).__init__()
        self.conv_in = nn.Conv3d(in_dim, dim, 3, 1, 1, bias=False)
        self.down1 = EdgeEnhanceModule(dim=dim)
        self.downsample1 = nn.Conv3d(dim, dim * 2, (1, 4, 4), (1, 2, 2), (0, 1, 1), bias=False)

        self.down2 = EdgeEnhanceModule(dim=dim * 2)
        self.downsample2 = nn.Conv3d(dim * 2, dim * 4, (1, 4, 4), (1, 2, 2), (0, 1, 1), bias=False)

        self.bottleneck_local = EdgeEnhanceModule(dim=dim * 2)
        self.bottleneck_swin = FAB_(dim=dim * 2, sq_dim=dim, dim_head=dim, mult=4, shift=True)

        self.upsample2 = nn.ConvTranspose3d(dim * 4, dim * 2, (1, 2, 2), (1, 2, 2))
        self.fusion2 = nn.Conv3d(dim * 4, dim * 2, 1, 1, 0, bias=False)
        self.up2 = FAB_(dim=dim * 2, sq_dim=dim, dim_head=dim, mult=4, shift=True)

        self.upsample1 = nn.ConvTranspose3d(dim * 2, dim, (1, 2, 2), (1, 2, 2))
        self.fusion1 = nn.Conv3d(dim * 2, dim, 1, 1, 0, bias=False)
        self.up1 = FAB_(dim=dim, sq_dim=dim, dim_head=dim, mult=4, shift=True)
    ...
    def forward(self, x, stage_ins=None, skip=False, stage_outs=None):
        ...
        theta = self.theta(x)
        # 通道 * 4
        x_local = self.bottleneck_local(x[:, :2*c, :, :]) * theta
        if stage_ins:
            stage_outs.append(self.bottleneck_local_action(x_local, stage_ins[2], stage_ins[3]))
        else:
            stage_outs.append(self.bottleneck_local_action(x_local, x_local, x_local))
        x_swin = self.bottleneck_swin(x[:, c*2:, :, :] + stage_outs[2]) * (1 - theta)
        stage_outs.append(x_swin)
        x = torch.cat([x_local, x_swin], dim=1)
        x = self.upsample2(x)
        x = x2 + self.fusion2(torch.cat([x, x2], dim=1))
...
class NetVideo_conv3d_TSAB_shareBody_action_changeSample_seriesConnection_withoutDP_deepcache_lightweight(
    torch.nn.Module):
    def __init__(self, opt):
        super(NetVideo_conv3d_TSAB_shareBody_action_changeSample_seriesConnection_withoutDP_deepcache_lightweight,
              self).__init__()
        ...
        self.net_stage_head = nn.ModuleList(nn.ModuleList([
            Conv_F_TSAB(in_dim=opt.dim, out_dim=opt.dim, dim=opt.dim),
            FB(in_dim=opt.dim * 2, dim=opt.dim)
        ]))
        self.net_stage_body_conv = nn.ModuleList([
            nn.ModuleList([
                Conv_F_TSAB(in_dim=opt.dim, out_dim=opt.dim, dim=opt.dim),
                FB(in_dim=opt.dim * 2, dim=opt.dim)
        ...
        self.net_stage_body = nn.ModuleList([
            nn.ModuleList([
                F_TSAB(in_dim=opt.dim, out_dim=opt.dim, dim=opt.dim),
                FB(in_dim=opt.dim * 2, dim=opt.dim)
            ]) for _ in range(self.stage - self.k - 1)
        ]) if not opt.body_share_params else nn.ModuleList([
            F_TSAB(in_dim=opt.dim, out_dim=opt.dim, dim=opt.dim),
            # DPB(in_dim=opt.dim * 2, dim=opt.dim),
            FB(in_dim=opt.dim * 2, dim=opt.dim)
    ...
    def forward(self, g, input_mask=None):
        ...
        out = []
        mu = self.mu[0](f)
        z, stage_outputs = self.net_stage_head[0](f)
        ...
        if not self.body_share_params:
            ...
            for i in range(self.k - 1):
                mu = self.mu[i + 1](f)
                z, stage_outputs = self.net_stage_body_conv[0](f, stage_outputs)
                # r = self.net_stage_body[1](torch.cat([z_ori - y / mu - f, f], dim=1), Phi, Phi_compressive)
                Phi_f = self.mul_Phif(Phi, z - y / mu)
                f = z - y / mu + self.mul_PhiTg(Phi, torch.div(g - Phi_f, mu + PhiPhiT))
                f = self.net_stage_body_conv[1](f, z)
                # z_ori = z
                y = y + mu * (f - z)
                out_ = self.vrm(f).squeeze(1)
                out.append(out_)
        ...
        if not self.body_share_params:
            for i in range(self.stage - self.k - 1):
                mu = self.mu[self.k + i](f)
                z, stage_outputs = self.net_stage_body[i][0](f, stage_outputs)
                # r = self.net_stage_body[i][1](torch.cat([z_ori - y / mu - f, f], dim=1), Phi, Phi_compressive)
                Phi_f = self.mul_Phif(Phi, z - y / mu)
                f = z - y / mu + self.mul_PhiTg(Phi, torch.div(g - Phi_f, mu + PhiPhiT))  # 有个reverse操作需要去掉
            ...
            for i in range(self.stage - self.k - 1):
                mu = self.mu[self.k + i](f)
                z, stage_outputs = self.net_stage_body[0](f, stage_outputs)
                # r = self.net_stage_body[1](torch.cat([z_ori - y / mu - f, f], dim=1), Phi, Phi_compressive)
                Phi_f = self.mul_Phif(Phi, z - y / mu)
                f = z - y / mu + self.mul_PhiTg(Phi, torch.div(g - Phi_f, mu + PhiPhiT))
        ...
        z, stage_outputs = self.net_stage_tail[0](f, stage_outputs)
