...
class Mu_Estimator(nn.Module):
    def __init__(self, in_nc=8, out_nc=1, channel=32):
...
class DPB(nn.Module):
    def __init__(self, in_dim=16):
        super().__init__()
        self.norm_n = nn.LayerNorm(16)
        self.norm_mask = nn.LayerNorm(16)
...
class NetVideo_FA(torch.nn.Module):
    def __init__(self, opt):
        super(NetVideo_FA, self).__init__()
        netlayer = []
        ...
        for i in range(opt.stage):
            netlayer.append(IPB(in_dim=16))
            netlayer.append(DPB(in_dim=16))
            netlayer.append(FB(in_dim=16))
    ...
    def forward(self, g, input_mask=None):
        ...
        Phi_compressive = self.reverse(Phi_compressive.repeat(1, 8, 1, 1))
        g_normal = g / self.nC * 2
        temp_g = g_normal.repeat(1, 8, 1, 1)
        ...
        for i in range(self.stage):
            ...
            f = z - r - y / mu + self.mul_PhiTg(Phi, torch.div(g - Phi_f, mu + PhiPhiT))
