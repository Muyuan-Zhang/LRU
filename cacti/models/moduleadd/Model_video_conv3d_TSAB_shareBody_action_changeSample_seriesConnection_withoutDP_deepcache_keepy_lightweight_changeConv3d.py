...
class MultiScaleFeatureFusion(nn.Module):
    def __init__(self, in_channels, out_channels):
        super().__init__()
        # 使用不同卷积核尺寸的并行卷积
        self.conv3d1 = nn.Conv3d(in_channels, out_channels, kernel_size=(1, 3, 3), padding=(0, 1, 1))
        self.leakyRelu = nn.LeakyReLU(inplace=True)
        self.conv3d2 = nn.Conv3d(in_channels, out_channels, kernel_size=(1, 3, 3), padding=(0, 1, 1))

    def forward(self, x):
        # 使用不同尺度的卷积提取特征
        x = self.conv3d2(self.leakyRelu(self.conv3d1(x))) + x
        return x


class EM_MF(nn.Module):
    def __init__(self, dim):
        super().__init__()
        self.em = EdgeEnhanceModule(dim=dim)
        self.mf = MultiScaleFeatureFusion(in_channels=dim, out_channels=dim)
...
class NetVideo_conv3d_TSAB_shareBody_action_changeSample_seriesConnection_withoutDP_deepcache_lightweight_changeConv3d(
    torch.nn.Module):
    def __init__(self, opt):
        super(
            NetVideo_conv3d_TSAB_shareBody_action_changeSample_seriesConnection_withoutDP_deepcache_lightweight_changeConv3d,
            self).__init__()
        ...
        self.net_stage_head = nn.ModuleList(nn.ModuleList([
            Conv_FAB(in_dim=opt.dim, out_dim=opt.dim, dim=opt.dim),
