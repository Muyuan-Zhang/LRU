...
class IPB(nn.Module):
    ...
    def forward(self, x, deepcache=None):
        """
            x: [b,c,B,h,w]
        ...
        if deepcache is None:
            ...
            x = self.up1(x)
            deepcache = self.conv_out(x)
            out = self.conv_out(x) + x_in
        else:
            out = deepcache + x_in
            deepcache = None
        return out[:, :, :h_inp, :w_inp], deepcache
...
class NetVideo_conv3d_TSAB_shareBody_deepcache(torch.nn.Module):
    def __init__(self, opt):
        super(NetVideo_conv3d_TSAB_shareBody_deepcache, self).__init__()
    ...
    def forward(self, g, input_mask=None):
        ...
        out = []
        mu = self.mu[0](f)
        z, deepcache = self.net_stage_head[0](torch.cat([f + y / mu + r, f], dim=1))
        ...
        if not self.body_share_params:
            for i in range(self.stage - 2):
                mu = self.mu[i + 1](f)
                z, deepcache = self.net_stage_body[i][0]((torch.cat([f + y / mu + r, f], dim=1)), deepcache)
            ...
            for i in range(self.stage - 2):
                mu = self.mu[i + 1](f)
                z, deepcache = self.net_stage_body[0]((torch.cat([f + y / mu + r, f], dim=1)), deepcache)
        ...
        z, deepcache = self.net_stage_tail[0]((torch.cat([f + y / mu + r, f], dim=1)), deepcache)
