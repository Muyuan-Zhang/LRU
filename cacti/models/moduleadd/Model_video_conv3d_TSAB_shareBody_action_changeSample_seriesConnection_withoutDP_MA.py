...
class MaskGuidedMechanism(nn.Module):
    def __init__(
        ...
        self.conv1 = nn.Conv3d(n_feat, n_feat, kernel_size=1, bias=True)
        self.conv2 = nn.Conv3d(n_feat, n_feat, kernel_size=1, bias=True)
        self.depth_conv = nn.Conv3d(n_feat, n_feat, kernel_size=5, padding=2, bias=True, groups=n_feat)
...
class FA(nn.Module):
    ...
    def cal_attention(self, x, mask=None):
        q, k = self.to_qk(x).chunk(2, dim=-1)
        v = self.to_v(x)
        q, k = map(lambda t: rearrange(t, 'b n B (h d) -> b h B n d', h=self.heads_qk), (q, k))
        v = rearrange(v, 'b n B (h d) -> b h B n d', h=self.heads_v)
        if mask is not None:
            mask_attn = self.mm(mask)
            mask_attn = rearrange(mask_attn, 'b n B (h d) -> b h B n d', h=self.heads_v)
            v = v * mask_attn
        attn = self.cal_atten(q, k)
    ...
    def forward(self, x, mask=None):
        ...
        if mask is not None:
            ...
            mask = rearrange(mask, 'b B (h b0) (w b1) c -> (b h w) (b0 b1) B c', b0=w_size[0], b1=w_size[1])
        out = self.cal_attention(x_inp, mask)
...
class IPB(nn.Module):
    def __init__(self, in_dim=8, out_dim=4, dim=4):
        ...
        self.mask_in = nn.Conv3d(dim, dim, 3, 1, 1, bias=False)
        ...
        self.downsample1_mask = nn.Conv3d(dim, dim * 2, (3, 4, 4), (1, 2, 2), (1, 1, 1), bias=False)
        ...
        self.downsample2_mask = nn.Conv3d(dim * 2, dim * 4, (3, 4, 4), (1, 2, 2), (1, 1, 1), bias=False)
        ...
        # self.upsample2_mask = nn.ConvTranspose3d(dim * 4, dim * 2, (1, 2, 2), (1, 2, 2))
        ...
        # self.upsample1_mask = nn.ConvTranspose3d(dim * 2, dim, (1, 2, 2), (1, 2, 2))
    ...
    def forward(self, x, mask, stage_ins=None, stage_outs=None):
        """
            x: [b,c,B,h,w]
        ...
        mask = self.downsample1_mask(mask)
        x2 = self.down2(x, mask)
        if stage_ins:
        ...
        mask = self.downsample2_mask(mask)
        # 通道 * 4
        x_local = self.bottleneck_local(x[:, :c, :, :], mask[:, :c, :, :])
        if stage_ins:
...
class TimesAttention3D(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        if mask is not None:
            mask_in = rearrange(mask, "b (h d) B H W -> (b H W) h B d", h=self.num_heads,
                                d=(mask.shape[1] // self.num_heads))
            v = v * mask_in
...
class NetVideo_conv3d_TSAB_shareBody_action_changeSample_seriesConnection_withoutDP_MA(torch.nn.Module):
    def __init__(self, opt):
        super(NetVideo_conv3d_TSAB_shareBody_action_changeSample_seriesConnection_withoutDP_MA, self).__init__()
    ...
    def forward(self, g, input_mask=None):
        ...
        Phi = Phi.unsqueeze(4)  # 形状[bs, 8, h, w, c=1]
        mask = Phi
        mask = mask.repeat(1, 1, 1, 1, self.dim).permute(0, 4, 1, 2, 3)
        ...
        out = []
        mu = self.mu[0](f)
        z, stage_outputs = self.net_stage_head[0](torch.cat([f, f], dim=1), mask)
        ...
        if not self.body_share_params:
            for i in range(self.stage - 2):
                mu = self.mu[i + 1](f)
                z, stage_outputs = self.net_stage_body[i][0](torch.cat([f, f], dim=1), mask, stage_outputs)
            ...
            for i in range(self.stage - 2):
                mu = self.mu[i + 1](f)
                z, stage_outputs = self.net_stage_body[0](torch.cat([f, f], dim=1), mask, stage_outputs)
                # r = self.net_stage_body[1](torch.cat([z_ori - y / mu - f, f], dim=1), Phi, Phi_compressive)
                Phi_f = self.mul_Phif(Phi, z / mu)
                f = z + self.mul_PhiTg(Phi, torch.div(g - Phi_f, mu + PhiPhiT))  # 有个reverse操作需要去掉
        ...
        z, stage_outputs = self.net_stage_tail[0](torch.cat([f, f], dim=1), mask, stage_outputs)
