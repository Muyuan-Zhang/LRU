...
class Spatial_FeedForward(nn.Module):
...
class Temporal_FeedForward(nn.Module):
    def __init__(self, dim, mult=4):
        super().__init__()
        self.net = nn.Sequential(
            nn.Conv3d(dim, dim * mult, 1, 1, bias=False),
...
class FAB(nn.Module):
    def __init__(self, dim, sq_dim, window_size=(8, 8), dim_head=28, mult=4, shift=False):
        ...
        self.norm2 = nn.LayerNorm(dim)
        # self.mpmlp = MPMLP(dim=dim, multi=mult)
        self.ffn = Temporal_FeedForward(dim=dim, mult=mult)
...
class TSAB(nn.Module):
    def __init__(self, dim, num_head):
        ...
        self.ffn = Temporal_FeedForward(dim=dim)
...
class NetVideo_conv3d_TSAB_shareBody_action_changeSample_seriesConnection_withoutDP_keep_y_ffn(torch.nn.Module):
    def __init__(self, opt):
        super(NetVideo_conv3d_TSAB_shareBody_action_changeSample_seriesConnection_withoutDP_keep_y_ffn, self).__init__()
