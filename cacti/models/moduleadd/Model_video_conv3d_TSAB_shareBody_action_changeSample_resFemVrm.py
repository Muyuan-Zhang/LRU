...
class IPB(nn.Module):
    def __init__(self, in_dim=8, out_dim=4, dim=4):
        ...
        self.down1 = FAB(dim=dim, sq_dim=dim, dim_head=dim, mult=4)
        ...
        self.down2 = FAB(dim=dim * 2, sq_dim=dim, dim_head=dim, mult=4)
        ...
        self.bottleneck_local = FAB(dim=dim * 2, sq_dim=dim, dim_head=dim, mult=4)
        self.bottleneck_swin = FAB(dim=dim * 2, sq_dim=dim, dim_head=dim, mult=4, shift=True)
        # 改变上采样，帧数不变
        ...
        self.up2 = FAB(dim=dim * 2, sq_dim=dim, dim_head=dim, mult=4, shift=True)
        # 改变上采样，帧数不变
        ...
        self.up1 = FAB(dim=dim, sq_dim=dim, dim_head=dim, mult=4, shift=True)
...
class FEM(nn.Module):
    def __init__(self, dim):
        ...
        self.fem_1 = nn.Sequential(
            nn.Conv3d(dim, dim, kernel_size=3, stride=1, padding=1),
            nn.LeakyReLU(inplace=True),
        )
        self.fem_2 = nn.Sequential(
            nn.Conv3d(dim, dim, kernel_size=3, stride=1, padding=1),
            nn.LeakyReLU(inplace=True),
        )
        self.fem_3 = nn.Sequential(
            nn.Conv3d(dim, dim, kernel_size=3, stride=1, padding=1),
            nn.LeakyReLU(inplace=True),
        )
...
class VRM(nn.Module):
    def __init__(self, dim):
        ...
        self.vrm_1 = nn.Sequential(
            nn.Conv3d(dim, dim, kernel_size=3, stride=1, padding=1),
            nn.LeakyReLU(inplace=True),
        )
        self.vrm_2 = nn.Sequential(
            nn.Conv3d(dim, dim, kernel_size=3, stride=1, padding=1),
            nn.LeakyReLU(inplace=True),
        )
        self.vrm_3 = nn.Sequential(
            nn.Conv3d(dim, dim, kernel_size=3, stride=1, padding=1),
            nn.LeakyReLU(inplace=True),
        )
        self.vrm_4 = nn.Conv3d(dim, 1, kernel_size=1, stride=1)

    def forward(self, x):
        x1 = self.vrm_1(x) + x
        x2 = self.vrm_2(x1) + x1
        x3 = self.vrm_3(x2) + x2
        return self.vrm_4(x3)


@MODELS.register_module
class NetVideo_conv3d_TSAB_shareBody_action_changeSample_resFemVrm(torch.nn.Module):
    def __init__(self, opt):
        super(NetVideo_conv3d_TSAB_shareBody_action_changeSample_resFemVrm, self).__init__()
        ...
        # 改动：添加一个特征提取块
        self.fem = FEM(dim=self.dim)

        # 特征恢复块
        self.vrm = VRM(dim=self.dim)
    ...
    def forward(self, g, input_mask=None):
        ...
        # 直接特征提取
        temp_g = self.fem(temp_g.permute(0, 4, 1, 2, 3).repeat(1, self.dim, 1, 1, 1))  # 形状[bs, c 8, h, w]
        g = g.permute(0, 4, 1, 2, 3)  # 形状[bs, c 8, h, w]
        Phi = self.fem(Phi.permute(0, 4, 1, 2, 3).repeat(1, self.dim, 1, 1, 1))  # 形状[bs, c 8, h, w]
        PhiPhiT = PhiPhiT.permute(0, 4, 1, 2, 3)  # 形状[bs, c 8, h, w]
        Phi_compressive = self.fem(Phi_compressive.permute(0, 4, 1, 2, 3).repeat(1, self.dim, 1, 1, 1))  # 形状[bs, c 8, h, w]

        # temp_g = temp_g.permute(0, 2, 3, 4, 1)

        f0 = temp_g
        # f = self.conv(torch.cat([f0, Phi], dim=1))
        ### f0 和 phi cat得到f 之后再特征提取（有问题）
        ...
        out = []
        mu = self.mu[0](f)
        z, stage_outputs = self.net_stage_head[0](torch.cat([f + y / mu + r, f], dim=1))
        r = self.net_stage_head[1](torch.cat([z_ori - y / mu - f, f], dim=1), Phi, Phi_compressive)
        Phi_f = self.mul_Phif(Phi, z - r - y / mu)
        f = z - r - y / mu + self.mul_PhiTg(Phi, torch.div(g - Phi_f, mu + PhiPhiT))  # 有个reverse操作需要去掉
        f = self.net_stage_head[2](f, z - r)
        z_ori = z
        y = y + mu * (f - z + r)
        ...
        if not self.body_share_params:
            for i in range(self.stage - 2):
                mu = self.mu[i + 1](f)
                z, stage_outputs = self.net_stage_body[i][0](torch.cat([f + y / mu + r, f], dim=1), stage_outputs)
                r = self.net_stage_body[i][1](torch.cat([z_ori - y / mu - f, f], dim=1), Phi, Phi_compressive)
                Phi_f = self.mul_Phif(Phi, z - r - y / mu)
                f = z - r - y / mu + self.mul_PhiTg(Phi, torch.div(g - Phi_f, mu + PhiPhiT))  # 有个reverse操作需要去掉
                f = self.net_stage_body[i][2](f, z - r)
                z_ori = z
                y = y + mu * (f - z + r)
                out_ = self.vrm(f).squeeze(1)
                out.append(out_)
            ...
            for i in range(self.stage - 2):
                mu = self.mu[i + 1](f)
                z, stage_outputs = self.net_stage_body[0](torch.cat([f + y / mu + r, f], dim=1), stage_outputs)
                r = self.net_stage_body[1](torch.cat([z_ori - y / mu - f, f], dim=1), Phi, Phi_compressive)
                Phi_f = self.mul_Phif(Phi, z - r - y / mu)
                f = z - r - y / mu + self.mul_PhiTg(Phi, torch.div(g - Phi_f, mu + PhiPhiT))  # 有个reverse操作需要去掉
                f = self.net_stage_body[2](f, z - r)
                z_ori = z
                y = y + mu * (f - z + r)
                out_ = self.vrm(f).squeeze(1)
                out.append(out_)
        ...
        z, stage_outputs = self.net_stage_tail[0](torch.cat([f + y / mu + r, f], dim=1), stage_outputs)
        r = self.net_stage_tail[1](torch.cat([z_ori - y / mu - f, f], dim=1), Phi, Phi_compressive)
        Phi_f = self.mul_Phif(Phi, z - r - y / mu)
        f = z - r - y / mu + self.mul_PhiTg(Phi, torch.div(g - Phi_f, mu + PhiPhiT))  # 有个reverse操作需要去掉
        f = self.net_stage_tail[2](f, z - r)
        # z_ori = z
        # y = y + mu * (f - z + r)
        out_ = self.vrm(f).squeeze(1)
        out.append(out_)
