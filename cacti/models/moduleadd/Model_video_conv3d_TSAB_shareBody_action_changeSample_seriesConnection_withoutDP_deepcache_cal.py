...
class NetVideo_conv3d_TSAB_shareBody_action_changeSample_seriesConnection_withoutDP_deepcache_cal(torch.nn.Module):
    def __init__(self, opt):
        super(NetVideo_conv3d_TSAB_shareBody_action_changeSample_seriesConnection_withoutDP_deepcache_cal,
              self).__init__()
        ...
        self.pool = GlobalMaxPoolingModel()
    ...
    def forward(self, g, input_mask=None, iteration=None):
        ...
        out = []
        mu = self.mu[0](f)
        z, stage_output = self.net_stage_head[0](torch.cat([f, f], dim=1))
        stage_outputs = []
        ...
        if not self.body_share_params:
            for i in range(self.stage - 2):
                mu = self.mu[i + 1](f)
                z, stage_output = self.net_stage_body[i][0](torch.cat([f, f], dim=1), stage_output)
                stage_outputs.append(stage_output)
            ...
            for i in range(self.stage - 2):
                mu = self.mu[i + 1](f)
                if self.stage == 5 and i == 2 or i == 0:
                    z, stage_output = self.net_stage_body[0](torch.cat([f, f], dim=1), stage_output, True)
                else:
                    z, stage_output = self.net_stage_body[0](torch.cat([f, f], dim=1), stage_output)
        ...
        z, stage_output = self.net_stage_tail[0](torch.cat([f, f], dim=1), stage_output)
        stage_outputs.append(stage_output)
        ...
        if iteration == 1:
            ...
            # 可视化
            output_tensor = self.pool(stage_outputs)
