...
class DW_Inception(nn.Module):
    def __init__(self, in_dim, dim):
        ...
        self.branch1 = nn.Sequential(
            ...
            nn.Conv3d(dim, in_dim, (1, 3, 3), 1, (0, 1, 1), bias=False),
            nn.InstanceNorm3d(dim),
            nn.GELU()
        )
        self.branch2 = nn.Sequential(
            nn.Conv3d(in_dim, dim, (1, 1, 1), 1, (0, 0, 0), bias=False),
            nn.InstanceNorm3d(dim),
            nn.Conv3d(dim, in_dim, (3, 1, 1), 1, (1, 0, 0), bias=False),
            nn.InstanceNorm3d(dim),
            nn.GELU()
        )
...
class IPB(nn.Module):
    def __init__(self, in_dim=8, out_dim=4, dim=4):
        ...
        self.downReuse1 = DW_Inception(dim, dim//2)
        self.downReuse2 = DW_Inception(dim * 2, dim//2)
        self.bottleneck_localReuse = DW_Inception(dim * 2, dim//2)
        self.bottleneck_swinReuse = DW_Inception(dim * 2, dim//2)
        self.upReuse2 = DW_Inception(dim * 2, dim//2)
        self.upReuse1 = DW_Inception(dim, dim//2)
    ...
    def forward(self, x, stage_ins=None, skip=False, stage_outs=None):
        ...
        if not skip:
            ...
            x_local = self.bottleneck_localReuse(x[:, :2 * c, :, :])
            stage_outs.append(self.bottleneck_local_action(x_local, stage_ins[2], stage_ins[3]))
        ...
        if not skip:
            ...
            x_swin = self.bottleneck_swinReuse(x[:, 2 * c:, :, :] + stage_outs[2])
            stage_outs.append(self.bottleneck_swin_action(x_swin, stage_ins[3], stage_ins[2]))
...
class NetVideo_conv3d_TSAB_shareBody_action_changeSample_seriesConnection_withoutDP_deepcache_keepy_reuse(
    torch.nn.Module):
    def __init__(self, opt):
        super(NetVideo_conv3d_TSAB_shareBody_action_changeSample_seriesConnection_withoutDP_deepcache_keepy_reuse,
              self).__init__()
