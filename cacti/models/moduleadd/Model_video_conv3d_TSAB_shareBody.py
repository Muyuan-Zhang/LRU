...
class NetVideo_conv3d_TSAB_shareBody(torch.nn.Module):
    def __init__(self, opt):
        super(NetVideo_conv3d_TSAB_shareBody, self).__init__()
    ...
    def forward(self, g, input_mask=None):
        ...
        out = []
        mu = self.mu[0](f)
        z = self.net_stage_head[0](torch.cat([f + y / mu + r, f], dim=1))
        ...
        if not self.body_share_params:
            for i in range(self.stage - 2):
                mu = self.mu[i + 1](f)
                z = self.net_stage_body[i][0](torch.cat([f + y / mu + r, f], dim=1))
            ...
            for i in range(self.stage - 2):
                mu = self.mu[i + 1](f)
                z = self.net_stage_body[0](torch.cat([f + y / mu + r, f], dim=1))
        ...
        z = self.net_stage_tail[0](torch.cat([f + y / mu + r, f], dim=1))
