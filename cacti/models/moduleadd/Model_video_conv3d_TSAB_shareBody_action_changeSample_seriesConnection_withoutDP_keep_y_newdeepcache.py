...
class IPB(nn.Module):
    ...
    def forward(self, x, stage_ins=None,skip=False, stage_outs=None):
        ...
        if not skip:
            ...
            # x = self.conv_in(x)
            x1 = self.down1(x)
            x = x1 + self.fusion1(torch.cat([stage_outs[5], x1], dim=1))
            stage_outs[5] = x
...
class NetVideo_conv3d_TSAB_shareBody_action_changeSample_seriesConnection_withoutDP_keep_y_newdeepcache(torch.nn.Module):
    def __init__(self, opt):
        super(NetVideo_conv3d_TSAB_shareBody_action_changeSample_seriesConnection_withoutDP_keep_y_newdeepcache, self).__init__()
    ...
    def forward(self, g, input_mask=None):
        ...
        if not self.body_share_params:
            ...
            for i in range(self.stage - 2):
                mu = self.mu[i + 1](f)
                if i % 2 == 0:
                    z, stage_outputs = self.net_stage_body[0](f, stage_output, True)
                else:
                    z, stage_outputs = self.net_stage_body[0](f, stage_output)
