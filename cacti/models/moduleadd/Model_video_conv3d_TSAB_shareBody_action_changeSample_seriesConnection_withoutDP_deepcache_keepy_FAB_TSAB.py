...
class FAB2(nn.Module):
    def __init__(self, dim, sq_dim, window_size=(8, 8), dim_head=28, mult=4, shift=False):
        super().__init__()
        self.FAB1 = FAB(dim=dim, sq_dim=sq_dim, window_size=window_size, dim_head=dim_head, mult=mult, shift=shift)
        self.FAB2 = FAB(dim=dim, sq_dim=sq_dim, window_size=window_size, dim_head=dim_head, mult=mult, shift=shift)
...
class TSAB2(nn.Module):
    def __init__(self, dim, sq_dim, window_size=(8, 8), dim_head=28, mult=4, shift=False):
        super().__init__()
        self.TSAB1 = TSAB(dim, num_head=dim_head)
...
class IPB(nn.Module):
    def __init__(self, in_dim=8, out_dim=4, dim=4):
        ...
        self.down1 = TSAB2(dim=dim, sq_dim=dim, dim_head=dim, mult=4)
        ...
        self.down2 = TSAB2(dim=dim * 2, sq_dim=dim, dim_head=dim, mult=4)
        ...
        self.bottleneck_local = TSAB2(dim=dim * 2, sq_dim=dim, dim_head=dim, mult=4)
        self.bottleneck_swin = TSAB2(dim=dim * 2, sq_dim=dim, dim_head=dim, mult=4, shift=True)
        # 改变上采样，帧数不变
        ...
        self.up2 = TSAB2(dim=dim * 2, sq_dim=dim, dim_head=dim, mult=4, shift=True)
        # 改变上采样，帧数不变
        ...
        self.up1 = TSAB2(dim=dim, sq_dim=dim, dim_head=dim, mult=4, shift=True)
...
class NetVideo_conv3d_TSAB_shareBody_action_changeSample_seriesConnection_withoutDP_deepcache_keepy_FAB_TSAB(
    torch.nn.Module):
    def __init__(self, opt):
        super(NetVideo_conv3d_TSAB_shareBody_action_changeSample_seriesConnection_withoutDP_deepcache_keepy_FAB_TSAB,
              self).__init__()
