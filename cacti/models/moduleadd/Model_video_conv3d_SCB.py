...
class FAB(nn.Module):
    ...
    def forward(self, x):
        ...
        x_ = self.norm1(x)
        x = self.fa(x_) + x + self.scb(x_.permute(0, 4, 1, 2, 3)).permute(0, 2, 3, 4, 1)
        x_ = self.norm2(x)
        x = self.mpmlp(x_) + x
...
class IPB(nn.Module):
    ...
    def forward(self, x):
        ...
        # 改动
        x_in = F.conv3d(x, self.shuffle_conv, groups=16)
        ...
        # 通道 * 4
        x_local = self.bottleneck_local(x[:, :32, :, :])
        x_swin = self.bottleneck_swin(x[:, 32:, :, :] + x_local)
...
class FB(nn.Module):
    def __init__(self, in_dim=32, dim=16):
        super().__init__()
        self.shuffle_conv = nn.Parameter(torch.cat([torch.ones(16, 1, 1, 1, 1), torch.zeros(16, 1, 1, 1, 1)], dim=1))
        self.out = nn.Sequential(
    ...
    def forward(self, f1, f2):
        ...
        out = F.conv3d(f, self.shuffle_conv, groups=16) + self.out(f)
        return out
...
class NetVideo_conv3d_SCB(torch.nn.Module):
    def __init__(self, opt):
        super(NetVideo_conv3d_SCB, self).__init__()
        netlayer = []
        ...
        for i in range(opt.stage):
            netlayer.append(IPB(in_dim=opt.dim * 2, out_dim=opt.dim, dim=opt.dim))
            netlayer.append(DPB(in_dim=opt.dim * 2, dim=opt.dim))
            netlayer.append(FB(in_dim=opt.dim * 2, dim=opt.dim))

        self.mu = nn.ModuleList(para_estimator)
        self.net_stage = nn.ModuleList(netlayer)

        # 改动：添加一个特征提取块
        self.fem = FEM(in_dim=1, dim=4)
    ...
    def forward(self, g, input_mask=None):
        ...
        f = self.conv3d(torch.cat([f0, Phi], dim=1))  # 在c或者8两种维度cat，需要更改conv3d的参数
