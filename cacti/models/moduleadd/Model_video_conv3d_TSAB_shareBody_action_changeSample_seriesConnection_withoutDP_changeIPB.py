...
class STSAB(nn.Module):
    def __init__(self, dim, window_size, dim_head, mult, shift):
        super().__init__()
        self.tsab = PreNorm(dim, SpatialTimesAttention3D(dim, window_size=window_size, dim_head=dim_head, shift=shift), norm_type='ln')
        self.norm2 = nn.LayerNorm(dim)
...
#         x = self.TSAB(x)
#         return x
class S_T_SAB(nn.Module):
    def __init__(self, dim, sq_dim, window_size=(8, 8), dim_head=28, mult=4, shift=False):
        super().__init__()
        self.STSAB = STSAB(dim=dim, window_size=window_size, dim_head=dim_head, mult=mult, shift=shift)
        # self.TSAB = TSAB(dim, num_head=dim_head)
...
class IPB(nn.Module):
    def __init__(self, in_dim=8, out_dim=4, dim=4):
        ...
        self.down1 = S_T_SAB(dim=dim, sq_dim=dim, dim_head=dim, mult=4)
        ...
        self.down2 = S_T_SAB(dim=dim * 2, sq_dim=dim, dim_head=dim, mult=4)
        ...
        self.bottleneck_local = S_T_SAB(dim=dim * 2, sq_dim=dim, dim_head=dim, mult=4)
        self.bottleneck_swin = S_T_SAB(dim=dim * 2, sq_dim=dim, dim_head=dim, mult=4, shift=True)
        # 改变上采样，帧数不变
        ...
        self.up2 = S_T_SAB(dim=dim * 2, sq_dim=dim, dim_head=dim, mult=4, shift=True)
        # 改变上采样，帧数不变
        ...
        self.up1 = S_T_SAB(dim=dim, sq_dim=dim, dim_head=dim, mult=4, shift=True)
...
class SpatialTimesAttention3D(nn.Module):
    def __init__(self, dim, window_size, dim_head, shift, qkv_bias=False, qk_scale=None):
        super().__init__()
        self.dim = dim
    ...
    def forward(self, x):
        ...
        if self.shift:
            x = x.roll(shifts=4, dims=1).roll(shifts=4, dims=2)
        tsab_in = rearrange(x, 'b B c (h b0) (w b1) -> (b h w) (b0 b1 B) c', b0=w_size[0], b1=w_size[1])
        n, B, C = tsab_in.shape
        ...
        attn = self.softmax(attn)
        x = (attn @ v).transpose(1, 2).reshape(n, B, C)
        x = rearrange(x, "(b h w) (B b0 b1) c-> b B (h b0) (w b1) c", h=h // w_size[0], w=w // w_size[1],
                        b0=w_size[0], b1=w_size[1], B=8)
        x = self.proj(x)
        if self.shift:
            x = x.roll(shifts=-4, dims=1).roll(shifts=-4, dims=2)
        x = x.permute(0, 4, 1, 2, 3)
...
class NetVideo_conv3d_TSAB_shareBody_action_changeSample_seriesConnection_withoutDP_changeIPB(torch.nn.Module):
    def __init__(self, opt):
        super(NetVideo_conv3d_TSAB_shareBody_action_changeSample_seriesConnection_withoutDP_changeIPB, self).__init__()
    ...
    def forward(self, g, input_mask=None):
        ...
        out = []
        mu = self.mu[0](f)
        z, stage_outputs = self.net_stage_head[0](torch.cat([f, f], dim=1))
        ...
        if not self.body_share_params:
            for i in range(self.stage - 2):
                mu = self.mu[i + 1](f)
                z, stage_outputs = self.net_stage_body[i][0](torch.cat([f, f], dim=1), stage_outputs)
            ...
            for i in range(self.stage - 2):
                mu = self.mu[i + 1](f)
                z, stage_outputs = self.net_stage_body[0](torch.cat([f, f], dim=1), stage_outputs)
        ...
        z, stage_outputs = self.net_stage_tail[0](torch.cat([f, f], dim=1), stage_outputs)
