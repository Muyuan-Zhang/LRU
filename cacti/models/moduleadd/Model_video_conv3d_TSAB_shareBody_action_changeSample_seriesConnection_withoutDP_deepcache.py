...
class NetVideo_conv3d_TSAB_shareBody_action_changeSample_seriesConnection_withoutDP_deepcache(torch.nn.Module):
    def __init__(self, opt):
        super(NetVideo_conv3d_TSAB_shareBody_action_changeSample_seriesConnection_withoutDP_deepcache, self).__init__()
        ...
        if not self.body_share_params:
            ...
            for i in range(self.stage - 2):
                ...
                if self.stage == 5 and i == 2 or i == 0:
                    z, stage_outputs = self.net_stage_body[0](torch.cat([f, f], dim=1), stage_outputs, True)
                else:
