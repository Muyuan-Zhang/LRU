...
class TimeAttention(nn.Module):
    ...
    def forward(self, q, k):
        ...
        diag_sim = torch.diagonal(sim, dim1=-2, dim2=-1)
        sim_norm = sim - torch.diag_embed(diag_sim)
        if self.mlp1 is None or self.mlp2 is None:
            input_dim = sim_norm.shape[-1]
            device = sim_norm.device
            self.mlp1 = nn.Sequential(
...
class FAB(nn.Module):
    def __init__(self, dim, sq_dim, window_size=(8, 8), dim_head=28, mult=4, shift=False):
        ...
        self.tsab = TimesAttention3D(dim, dim_head=dim_head)
...
class IPB(nn.Module):
    def __init__(self, in_dim=8, out_dim=4, dim=4):
        ...
        self.upsample2 = nn.ConvTranspose3d(dim * 4, dim * 2, 2, 2)
...
class TimesAttention3D(nn.Module):
    def __init__(self, dim, dim_head, qkv_bias=False, qk_scale=None):
        super().__init__()
        self.dim = dim
        ...
        self.cal_atten = TimeAttention(dim_head)
...
class NetVideo_conv3d_FTSAB_shareBody_action_changeInput(torch.nn.Module):
    def __init__(self, opt):
        super(NetVideo_conv3d_FTSAB_shareBody_action_changeInput, self).__init__()
