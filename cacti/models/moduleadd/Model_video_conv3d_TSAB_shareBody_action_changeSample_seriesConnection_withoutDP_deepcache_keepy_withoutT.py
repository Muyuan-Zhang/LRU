...
class IPB_withoutT(nn.Module):
    def __init__(self, in_dim=8, out_dim=4, dim=4):
        super(IPB_withoutT, self).__init__()
        self.conv_in = nn.Conv3d(in_dim, dim, 3, 1, 1, bias=False)

        self.down1 = FAB_SCB(dim=dim, sq_dim=dim, dim_head=dim, mult=4)
        self.downsample1 = nn.Conv3d(dim, dim * 2, (3, 4, 4), (1, 2, 2), (1, 1, 1), bias=False)

        self.down2 = FAB_SCB(dim=dim * 2, sq_dim=dim, dim_head=dim, mult=4)
        self.downsample2 = nn.Conv3d(dim * 2, dim * 4, (3, 4, 4), (1, 2, 2), (1, 1, 1), bias=False)

        self.bottleneck_local = FAB_SCB(dim=dim * 2, sq_dim=dim, dim_head=dim, mult=4)
        self.bottleneck_swin = FAB_SCB(dim=dim * 2, sq_dim=dim, dim_head=dim, mult=4, shift=True)

        self.upsample2 = nn.ConvTranspose3d(dim * 4, dim * 2, (1, 2, 2), (1, 2, 2))
        self.fusion2 = nn.Conv3d(dim * 4, dim * 2, 1, 1, 0, bias=False)
        self.up2 = FAB_SCB(dim=dim * 2, sq_dim=dim, dim_head=dim, mult=4, shift=True)

        self.upsample1 = nn.ConvTranspose3d(dim * 2, dim, (1, 2, 2), (1, 2, 2))
        self.fusion1 = nn.Conv3d(dim * 2, dim, 1, 1, 0, bias=False)
        self.up1 = FAB_SCB(dim=dim, sq_dim=dim, dim_head=dim, mult=4, shift=True)
...
class NetVideo_conv3d_TSAB_shareBody_action_changeSample_seriesConnection_withoutDP_deepcache_keepy_withoutT(torch.nn.Module):
    def __init__(self, opt):
        super(NetVideo_conv3d_TSAB_shareBody_action_changeSample_seriesConnection_withoutDP_deepcache_keepy_withoutT,
              self).__init__()
        ...
        self.net_stage_head = nn.ModuleList(nn.ModuleList([
            IPB_withoutT(in_dim=opt.dim, out_dim=opt.dim, dim=opt.dim),
            # DPB(in_dim=opt.dim * 2, dim=opt.dim),
            FB(in_dim=opt.dim * 2, dim=opt.dim)
        ]))
        self.net_stage_body_withoutT = nn.ModuleList([
            nn.ModuleList([
                IPB_withoutT(in_dim=opt.dim, out_dim=opt.dim, dim=opt.dim),
                # DPB(in_dim=opt.dim * 2, dim=opt.dim),
                FB(in_dim=opt.dim * 2, dim=opt.dim)
    ...
    def forward(self, g, input_mask=None):
        ...
        if not self.body_share_params:
            for i in range(self.k - 1):
                mu = self.mu[i + 1](f)
                z, stage_outputs = self.net_stage_body_withoutT[i][0](f, stage_outputs)
                # r = self.net_stage_body[i][1](torch.cat([z_ori - y / mu - f, f], dim=1), Phi, Phi_compressive)
                Phi_f = self.mul_Phif(Phi, z - y / mu)
                f = z - y / mu + self.mul_PhiTg(Phi, torch.div(g - Phi_f, mu + PhiPhiT))  # 有个reverse操作需要去掉
                f = self.net_stage_body_withoutT[i][1](f, z)
                # z_ori = z
                y = y + mu * (f - z)
                out_ = self.vrm(f).squeeze(1)
                out.append(out_)
            ...
            for i in range(self.k - 1):
                mu = self.mu[i + 1](f)
                z, stage_outputs = self.net_stage_body_withoutT[0](f, stage_outputs)
                # r = self.net_stage_body[1](torch.cat([z_ori - y / mu - f, f], dim=1), Phi, Phi_compressive)
                Phi_f = self.mul_Phif(Phi, z - y / mu)
                f = z - y / mu + self.mul_PhiTg(Phi, torch.div(g - Phi_f, mu + PhiPhiT))  # 有个reverse操作需要去掉
                f = self.net_stage_body_withoutT[1](f, z)
                # z_ori = z
                y = y + mu * (f - z)
                out_ = self.vrm(f).squeeze(1)
                out.append(out_)
