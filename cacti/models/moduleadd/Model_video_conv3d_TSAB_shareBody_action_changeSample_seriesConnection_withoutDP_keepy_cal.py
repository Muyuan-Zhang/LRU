...
class NetVideo_conv3d_TSAB_shareBody_action_changeSample_seriesConnection_withoutDP_keepy(torch.nn.Module):
    def __init__(self, opt):
        super(NetVideo_conv3d_TSAB_shareBody_action_changeSample_seriesConnection_withoutDP_keepy, self).__init__()
    ...
    def forward(self, g, input_mask=None, iteration=None):
        ...
        out_ = self.vrm(f).squeeze(1)
        out.append(out_)
        stage_outputs.append(out_.squeeze(0))
        # 可视化图片
        if iteration == 5:
            # psnr_dict, ssim_dict = eval_stage_psnr_ssim(stage_outputs)
            # 可视化
            output_tensor = self.pool_stage_outputs(stage_outputs)
            f_tensor = self.pool_f_outputs(fs)
            # 将结果转换为 NumPy 数组，方便使用 matplotlib 可视化
            for i in range(6):
                for j in range(6):
                    for k in range(8):
                        if i == 5:
                            plt.imshow(output_tensor[i][k].detach().cpu().numpy(), cmap='gray')  # 使用灰度图显示
                            plt.colorbar()  # 添加颜色条
                            plt.title(f"Result stage_{i + 1}module_{j + 1}frame_{k + 1}")
                            # 保存图片，指定文件名和格式
                            output_filename = "/home/yychen/zhangmuyuan/DPU/work_dirs/DPU_base/stage_image_test/"
                            output_filename = output_filename + f"stage_{i + 1}module_{j + 1}frame_{k + 1}.png"
                            plt.savefig(output_filename)
                            # 清空当前图形，以便下次循环时绘制新的图片
                            plt.clf()
                        else:
                            plt.imshow(output_tensor[i][j][k].detach().cpu().numpy(), cmap='gray')  # 使用灰度图显示
                            plt.colorbar()  # 添加颜色条
                            plt.title(f"Result stage_{i + 1}module_{j + 1}frame_{k + 1}")
                            # 保存图片，指定文件名和格式
                            output_filename = "/home/yychen/zhangmuyuan/DPU/work_dirs/DPU_base/stage_image_test/"
                ...
                for l in range(2):
                    for k in range(8):
                        plt.imshow(f_tensor[i][l][k].detach().cpu().numpy(), cmap='gray')  # 使用灰度图显示
                        plt.colorbar()  # 添加颜色条
                        plt.title(f"Result stage_{i + 1}stf_{l + 1}frame_{k + 1}")
                        # 保存图片，指定文件名和格式
                        output_filename = "/home/yychen/zhangmuyuan/DPU/work_dirs/DPU_base/stage_image_test/"
                        output_filename = output_filename + f"stage_{i + 1}stf_{l + 1}frame_{k + 1}.png"
                        plt.savefig(output_filename)
                        # 清空当前图形，以便下次循环时绘制新的图片
                        plt.clf()
        return out
