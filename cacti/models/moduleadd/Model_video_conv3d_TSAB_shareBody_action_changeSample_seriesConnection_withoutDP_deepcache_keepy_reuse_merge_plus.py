...
class FeedForward_reuse(nn.Module):
    def __init__(self, dim, mult=4):
        super().__init__()
        self.net = nn.Sequential(
            nn.Conv3d(dim, dim * mult, 1, 1, bias=False),
...
class LightweightConv3d(nn.Module):
    def __init__(self, in_dim, dim):
        super().__init__()

        # self.ffn1 = FeedForward_reuse(dim)
        self.branch1 = nn.Sequential(
            nn.Conv3d(dim, dim, (3, 1, 1), 1, (1, 0, 0), bias=False),
            nn.GroupNorm(dim, dim),
            nn.LeakyReLU(negative_slope=0.1, inplace=True),
            nn.Conv3d(in_dim, dim, (3, 1, 1), 1, (1, 0, 0), bias=False),
            nn.GroupNorm(dim, dim),
            nn.LeakyReLU(negative_slope=0.1, inplace=True),
        )
        self.branch2 = nn.Sequential(
            nn.Conv3d(in_dim, in_dim, (1, 3, 3), 1, (0, 1, 1), bias=False),
            nn.GroupNorm(in_dim, in_dim),
            nn.LeakyReLU(negative_slope=0.1, inplace=True),
            nn.Conv3d(dim, dim, (1, 3, 3), 1, (0, 1, 1), bias=False),
            nn.GroupNorm(dim, dim),
            nn.LeakyReLU(negative_slope=0.1, inplace=True),
        )

        # self.ffn2 = FeedForward_reuse(dim)
...
class Reuse(nn.Module):
    def __init__(self, in_dim, out_dim, dim):
        ...
        self.in_dim = in_dim
        self.dim = in_dim
        self.shuffle_conv = nn.Parameter(torch.cat(
            [torch.ones(in_dim, 1, 1, 1, 1),
             torch.zeros(in_dim, 1, 1, 1, 1), torch.zeros(in_dim, 1, 1, 1, 1)], dim=1))
        self.conv_enc = nn.Conv3d(in_dim, in_dim, 1, 1, 0)
        self.conv_dec = nn.Conv3d(in_dim, in_dim, 1, 1, 0)

        self.lightweight_conv = LightweightConv3d(in_dim, in_dim)

        # self.conv = nn.Conv3d(dim, out_dim, (1, 1, 1), 1, (0, 0, 0), bias=False)
        self.stageInteraction = StageInteraction(in_dim)

        # self.shuffle_conv_out = nn.Parameter(torch.cat(
        #     [torch.ones(in_dim, 1, 1, 1, 1),
        #      torch.ones(in_dim, 1, 1, 1, 1), torch.ones(in_dim, 1, 1, 1, 1)], dim=1))
    ...
    def forward(self, x, enc, dec):
        enc = self.conv_enc(enc)
        dec = self.conv_dec(dec)
        x1 = rearrange(torch.cat([x, enc, dec], 1), 'b (n c) B h w -> b (c n) B h w', n=3)  # in_dim * 3
        x1 = F.conv3d(x1, self.shuffle_conv, groups=self.in_dim)  # in_dim
        x2 = self.stageInteraction(x1, enc, dec)
        x3 = self.lightweight_conv(x2)  #
        # x = rearrange(torch.cat([x1, x2, x3], 1), 'b (n c) B h w -> b (c n) B h w', n=3)  # in_dim * 3
        out = x3  # in_dim
...
class IPB_reuse(nn.Module):
    def __init__(self, in_dim=8, out_dim=4, dim=4):
        super(IPB_reuse, self).__init__()
        ...
        self.downReuse1 = Reuse(dim, dim, dim)
        self.downReuse2 = Reuse(dim * 2, dim * 2, dim)
        self.bottleneck_localReuse = Reuse(dim * 2, dim * 2, dim)
        self.bottleneck_swinReuse = Reuse(dim * 2, dim * 2, dim)
        self.upReuse2 = Reuse(dim * 2, dim * 2, dim)
        self.upReuse1 = Reuse(dim, dim, dim)
    ...
    def forward(self, x, stage_ins=None, skip=False, stage_outs=None):
        ...
        if not skip:
            ...
            # x1 = self.downReuse1(torch.cat([x, stage_ins[0], stage_ins[5]], 1))
            x1 = self.downReuse1(x, stage_ins[0], stage_ins[5])
            # stage_outs.append(self.down1_action(x1, stage_ins[0], stage_ins[5]))
        ...
        if not skip:
            ...
            # x2 = self.downReuse2(torch.cat([x, stage_ins[1], stage_ins[4]], 1))
            x2 = self.downReuse2(x, stage_ins[1], stage_ins[4])
            # stage_outs.append(self.down2_action(x2, stage_ins[1], stage_ins[4]))
            stage_outs.append(x2)
        x = self.downsample2(stage_outs[1])
        # 通道 * 4
        if not skip:
            x_local = self.bottleneck_local(x[:, :2 * c, :, :])
            ...
            # x_local = self.bottleneck_localReuse(torch.cat([x[:, :2 * c, :, :], stage_ins[2], stage_ins[3]], 1))
            x_local = self.bottleneck_localReuse(x[:, :2 * c, :, :], stage_ins[2], stage_ins[3])
            # stage_outs.append(self.bottleneck_local_action(x_local, stage_ins[2], stage_ins[3]))
            stage_outs.append(x_local)
        ...
        if not skip:
            ...
            # x_swin = self.bottleneck_swinReuse(torch.cat([x[:, 2 * c:, :, :] + stage_outs[2], stage_ins[3], stage_ins[2]], 1))
            x_swin = self.bottleneck_swinReuse(x[:, 2 * c:, :, :] + stage_outs[2], stage_ins[3], stage_ins[2])
            # stage_outs.append(self.bottleneck_swin_action(x_swin, stage_ins[3], stage_ins[2]))
        ...
        if not skip:
            ...
            # x = self.upReuse2(torch.cat([x, stage_ins[4], stage_ins[1]], 1))
            x = self.upReuse2(x, stage_ins[4], stage_ins[1])
            # stage_outs.append(self.up2_action(x, stage_ins[4], stage_ins[1]))
            stage_outs.append(x)
        ...
        if not skip:
            ...
            # x = self.upReuse1(torch.cat([x, stage_ins[5], stage_ins[0]], 1))
            x = self.upReuse1(x, stage_ins[5], stage_ins[0])
            # stage_outs.append(self.up1_action(x, stage_ins[5], stage_ins[0]))
            stage_outs.append(x)
...
class NetVideo_conv3d_TSAB_shareBody_action_changeSample_seriesConnection_withoutDP_deepcache_keepy_reuse_merge_plus(
    torch.nn.Module):
    def __init__(self, opt):
        super(
            NetVideo_conv3d_TSAB_shareBody_action_changeSample_seriesConnection_withoutDP_deepcache_keepy_reuse_merge_plus,
            self).__init__()
        # assert opt.stage == len(opt.reuse)
        self.stage = opt.stage
        self.reuse = opt.reuse
        self.segment = len(opt.reuse)
        self.nC = opt.bands
        self.dim = opt.dim
        self.size = opt.size
        ...
        for i in range(self.segment):
            if i == 0 or i == self.stage - 1 or i == 2:
                ...
                self.net.append(nn.ModuleList([
                    IPB_reuse(in_dim=opt.dim, out_dim=opt.dim, dim=opt.dim),
                    FB(in_dim=opt.dim * 2, dim=opt.dim)
                ]))
    ...
    def forward(self, g, input_mask=None):
        ...
        stage_outputs = None
        for i in range(self.segment):
            for j in range(self.reuse[i]):
                mu = self.mu[k](f)
                k = k + 1
                # if j == 0 or i == self.segment - 1:
                if i == 0 or i == self.segment - 1 or i == 2:
                    z, stage_outputs = self.net[i][0](f, stage_outputs)
                else:
                    z, stage_outputs = self.net[i][0](f, stage_outputs, True)
                # r = self.net_stage_head[1](torch.cat([z_ori - y / mu - f, f], dim=1), Phi, Phi_compressive)
                Phi_f = self.mul_Phif(Phi, z - y / mu)
