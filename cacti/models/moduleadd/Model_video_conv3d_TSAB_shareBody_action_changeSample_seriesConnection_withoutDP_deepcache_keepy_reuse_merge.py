...
class Reuse(nn.Module):
    def __init__(self, in_dim, out_dim, dim):
        ...
        self.in_dim = in_dim
        self.shuffle_conv = nn.Parameter(torch.cat([torch.ones(in_dim//2, 1, 1, 1, 1), torch.zeros(in_dim//2, 1, 1, 1, 1)], dim=1))

        self.branch1 = nn.Sequential(
            nn.Conv3d(in_dim, in_dim, (1, 1, 1), 1, (0, 0, 0), bias=False),
            nn.GroupNorm(in_dim, in_dim),
            nn.LeakyReLU(negative_slope=0.1, inplace=True),
            nn.Conv3d(in_dim, dim, (1, 1, 1), 1, (0, 0, 0), bias=False),
            nn.GroupNorm(dim, dim),
            nn.LeakyReLU(negative_slope=0.1, inplace=True),
        )
        self.branch2 = nn.Sequential(
            nn.Conv3d(dim, dim, (1, 1, 1), 1, (0, 0, 0), bias=False),
            nn.GroupNorm(dim, dim),
            nn.LeakyReLU(negative_slope=0.1, inplace=True),
            nn.Conv3d(dim, out_dim, (1, 1, 1), 1, (0, 0, 0), bias=False),
            nn.GroupNorm(out_dim, out_dim),
            nn.LeakyReLU(negative_slope=0.1, inplace=True),
        )

    def forward(self, x):
        x = rearrange(x, 'b (n c) B h w -> b (c n) B h w', n=2)
        x3 = F.conv3d(x, self.shuffle_conv, groups=self.in_dim//2)
        x1 = self.branch1(x)
        x2 = self.branch2(x1)
        out = x2 + x3
        return out
...
class IPB_last(nn.Module):
    def __init__(self, in_dim=8, out_dim=4, dim=4):
        super(IPB_last, self).__init__()
...
class IPB(nn.Module):
    def __init__(self, in_dim=8, out_dim=4, dim=4):
        ...
        self.downReuse1 = Reuse(dim * 2, dim, dim)
        self.downReuse2 = Reuse(dim * 4, dim * 2, dim)
        self.bottleneck_localReuse = Reuse(dim * 4, dim * 2, dim)
        self.bottleneck_swinReuse = Reuse(dim * 4, dim * 2, dim)
        self.upReuse2 = Reuse(dim * 4, dim * 2, dim)
        self.upReuse1 = Reuse(dim * 2, dim, dim)
    ...
    def forward(self, x, stage_ins=None, skip=False, stage_outs=None):
        ...
        if not skip:
            ...
            x1 = self.downReuse1(torch.cat([x, stage_ins[0]], 1))
            stage_outs.append(self.down1_action(x1, stage_ins[0], stage_ins[5]))
        x = self.downsample1(stage_outs[0])
        if not skip:
            x2 = self.down2(x)
            ...
            x2 = self.downReuse2(torch.cat([x, stage_ins[1]], 1))
            stage_outs.append(self.down2_action(x2, stage_ins[1], stage_ins[4]))
        x = self.downsample2(stage_outs[1])
        # 通道 * 4
        if not skip:
            ...
            x_local = self.bottleneck_localReuse(torch.cat([x[:, :2 * c, :, :], stage_ins[2]], 1))
            stage_outs.append(self.bottleneck_local_action(x_local, stage_ins[2], stage_ins[3]))
        ...
        if not skip:
            ...
            x_swin = self.bottleneck_swinReuse(torch.cat([x[:, 2 * c:, :, :] + stage_outs[2], stage_ins[3]], 1))
            stage_outs.append(self.bottleneck_swin_action(x_swin, stage_ins[3], stage_ins[2]))
        ...
        if not skip:
            ...
            x = self.upReuse2(torch.cat([x, stage_ins[4]], 1))
            stage_outs.append(self.up2_action(x, stage_ins[4], stage_ins[1]))
        ...
        if not skip:
            ...
            x = self.upReuse1(torch.cat([x, stage_ins[5]], 1))
            stage_outs.append(self.up1_action(x, stage_ins[5], stage_ins[0]))
        out = self.conv_out(stage_outs[5]) + x_in
...
class NetVideo_conv3d_TSAB_shareBody_action_changeSample_seriesConnection_withoutDP_deepcache_keepy_reuse_merge(
    torch.nn.Module):
    def __init__(self, opt):
        super(NetVideo_conv3d_TSAB_shareBody_action_changeSample_seriesConnection_withoutDP_deepcache_keepy_reuse_merge,
              self).__init__()
        ...
        for i in range(self.segment):
            if i == self.segment - 1:
                self.net.append(nn.ModuleList([
                    IPB_last(in_dim=opt.dim, out_dim=opt.dim, dim=opt.dim),
