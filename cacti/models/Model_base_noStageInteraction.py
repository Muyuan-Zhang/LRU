...
class NetVideo_base_noStageInteraction(torch.nn.Module):
    def __init__(self, opt):
        super(NetVideo_base_noStageInteraction, self).__init__()
    ...
    def forward(self, g, input_mask=None):
        ...
        z = self.net_stage_head[0](torch.cat([f + y / mu, f], dim=1))
        ...
        if not self.body_share_params:
            for i in range(self.stage - 2):
                mu = self.mu[i + 1](f)
                z = self.net_stage_body[i][0](torch.cat([f + y / mu, f], dim=1))
            ...
            for i in range(self.stage - 2):
                mu = self.mu[i + 1](f)
                z = self.net_stage_body[0](torch.cat([f + y / mu, f], dim=1))
        ...
        z = self.net_stage_tail[0](torch.cat([f + y / mu, f], dim=1))
