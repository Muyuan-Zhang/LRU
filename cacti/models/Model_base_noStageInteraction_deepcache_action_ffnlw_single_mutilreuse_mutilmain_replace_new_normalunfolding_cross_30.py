...
class FA_inter(nn.Module):
    def __init__(self, dim, window_size=(8, 8), dim_head=28, sq_dim=None, shift=True):
        ...
        self.to_skip = nn.Conv3d(dim, dim, 1, 1, bias=False)
        self.act_fn = nn.LeakyReLU()
        self.to_phi = nn.Conv3d(dim, dim, (1, 3, 3), 1, padding=(0, 1, 1), bias=False, groups=dim)
        self.to_gamma = nn.Conv3d(dim, dim, (1, 3, 3), 1, padding=(0, 1, 1), bias=False, groups=dim)
        # self.to_out = nn.Linear(dim, dim)

    def cal_attention(self, x, last):
        w_size = self.window_size
        q = self.to_q(x.permute(0, 4, 1, 2, 3)).permute(0, 2, 3, 4, 1).contiguous()
        k = self.to_k(last.permute(0, 4, 1, 2, 3)).permute(0, 2, 3, 4, 1).contiguous()
        v = self.to_v(last.permute(0, 4, 1, 2, 3)).permute(0, 2, 3, 4, 1).contiguous()
    ...
    def forward(self, x, last):
        ...
        out = rearrange(out, '(b h w) (b0 b1) B c -> b B (h b0) (w b1) c', h=h // w_size[0], w=w // w_size[1],
                        b0=w_size[0])
        out = self.to_out(out.permute(0, 4, 1, 2, 3))
        skip = self.to_skip(self.act_fn(last.permute(0, 4, 1, 2, 3)))
        phi = torch.sigmoid(self.to_phi(skip))
        gamma = self.to_gamma(skip)
        out = phi * out + gamma
...
class TimesAttention3D_inter(nn.Module):
    def __init__(self, dim, num_head, qkv_bias=False, qk_scale=None):
        ...
        self.skip = nn.Conv3d(dim, dim, kernel_size=1, padding=0, bias=qkv_bias)
        self.act_fn = nn.LeakyReLU()
        self.phi = nn.Conv3d(dim, dim, kernel_size=(1, 3, 3), padding=(0, 1, 1), bias=qkv_bias, groups=dim)
        self.gamma = nn.Conv3d(dim, dim, kernel_size=(1, 3, 3), padding=(0, 1, 1), bias=qkv_bias, groups=dim)
    ...
    def forward(self, x, last):
        ...
        q = self.q(x).permute(0, 2, 1, 3, 4)
        k = self.k(last).permute(0, 2, 1, 3, 4)
        v = self.v(last).permute(0, 2, 1, 3, 4)
...
class NetVideo_base_noStageInteraction_deepcache_action_ffnlw_single_multireuse_multimain_replace_new_normalunfolding_cross_30(
    torch.nn.Module):
    def __init__(self, opt):
        super(
            NetVideo_base_noStageInteraction_deepcache_action_ffnlw_single_multireuse_multimain_replace_new_normalunfolding_cross_30,
            self).__init__()
