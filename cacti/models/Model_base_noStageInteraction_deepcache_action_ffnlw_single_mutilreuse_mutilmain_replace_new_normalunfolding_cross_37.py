...
class TSAB_inter(nn.Module):
    ...
    def forward(self, x, last):
        ...
        skip = self.act_fn(self.conv1(last))
        x = x * self.phi(torch.sigmoid(skip)) + self.gamma(skip)
...
class NetVideo_base_noStageInteraction_deepcache_action_ffnlw_single_multireuse_multimain_replace_new_normalunfolding_cross_37(
    torch.nn.Module):
    def __init__(self, opt):
        super(
            NetVideo_base_noStageInteraction_deepcache_action_ffnlw_single_multireuse_multimain_replace_new_normalunfolding_cross_37,
            self).__init__()
