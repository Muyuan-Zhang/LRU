...
class TSAB_inter(nn.Module):
    ...
    def forward(self, x, last):
        ...
        x = self.tsab_inter(x_, last) + x * torch.sigmoid(self.act_fn(self.conv(last)))
        # x = x.permute(0, 2, 3, 4, 1)  # 把通道放在后面
        # x = self.ffn(self.norm2(x)) + x
        # x = x.permute(0, 4, 1, 2, 3)
        return x
...
class NetVideo_base_noStageInteraction_deepcache_action_ffnlw_single_multireuse_multimain_replace_new_normalunfolding_cross_36(
    torch.nn.Module):
    def __init__(self, opt):
        super(
            NetVideo_base_noStageInteraction_deepcache_action_ffnlw_single_multireuse_multimain_replace_new_normalunfolding_cross_36,
            self).__init__()
