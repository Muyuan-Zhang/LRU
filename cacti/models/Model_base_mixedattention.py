...
class TimesAttention3D(nn.Module):
    def __init__(self, dim, num_head, qkv_bias=False, qk_scale=None):
        ...
        self.k_spatial = nn.Conv3d(dim, dim * 9, (1, 3, 3), 1, (0, 1, 1), bias=qkv_bias, groups=dim)
        self.v_spatial = nn.Conv3d(dim, dim * 9, (1, 3, 3), 1, (0, 1, 1), bias=qkv_bias, groups=dim)
    ...
    def forward(self, x):
        ...
        k_spatial = self.k_spatial(tsab_in)
        v_spatial = self.v_spatial(tsab_in)
...
class NetVideo_base(torch.nn.Module):
    def __init__(self, opt):
        super(NetVideo_base, self).__init__()
