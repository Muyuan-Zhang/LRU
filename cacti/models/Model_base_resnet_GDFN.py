...
#             nn.Conv3d(in_dim, out_dim, 1, 1, 0, bias=False),
#             nn.GELU(),
#             nn.Conv3d(in_dim, out_dim, (1, 3, 3), 1, (0, 1, 1), bias=False),
#             nn.GELU(),
...
#         return x1
class lightweight_module_time(nn.Module):
    def __init__(self, in_dim, out_dim):
        super(lightweight_module_time, self).__init__()
        self.ln1 = nn.LayerNorm(in_dim)
        self.branch1 = nn.Sequential(
            nn.Conv3d(in_dim, out_dim, 1, 1, 0, bias=False),
            nn.Conv3d(out_dim, out_dim, (3, 1, 1), 1, (1, 0, 0), bias=False),
        )
        # self.shortcut1 = nn.Conv3d(in_dim, out_dim, 1, 1, 0, bias=False)
        self.ln2 = nn.LayerNorm(in_dim)
        self.branch2 = nn.Sequential(
        ...
        self.conv = nn.Conv3d(in_dim, out_dim, 1, 1, 0, bias=False)
        # self.shortcut2 = nn.Conv3d(in_dim, out_dim, 1, 1, 0, bias=False)
    ...
    def forward(self, x):
        ...
        x = self.ln1(x)
        x = x.permute(0, 4, 1, 2, 3)
        x = x + self.conv(self.branch1(x) * self.branch2(x))
        return x


# class lightweight_module(torch.nn.Module):
#     def __init__(self, in_channels, hidden_dim, is_train):
#         super().__init__()
#         hidden_dim = in_channels * 4
#         self.in_conv1x1 = nn.Conv3d(in_channels, in_channels * 2, kernel_size=1, stride=1, padding=0, bias=False)
#         self.expand_conv1x1 = nn.Conv3d(in_channels * 2, hidden_dim, kernel_size=1, stride=1, padding=0, bias=False)
#
#         # 多尺度深度卷积
#         self.dwconv_k1 = nn.Conv3d(hidden_dim, hidden_dim, kernel_size=1, stride=1, padding=0, groups=hidden_dim, bias=False)
#         self.dwconv_k3 = nn.Conv3d(hidden_dim, hidden_dim, kernel_size=3, stride=1, padding=1, groups=hidden_dim, bias=False)
#         self.dwconv_k5 = nn.Conv3d(hidden_dim, hidden_dim, kernel_size=5, stride=1, padding=2, groups=hidden_dim, bias=False)
#         self.identity = nn.Identity()
#
#         # 缩减通道数的卷积
#         self.out_conv1x1 = nn.Conv3d(hidden_dim, in_channels * 2, kernel_size=1, stride=1, padding=0, bias=False)
#         self.reduce_conv1x1 = nn.Conv3d(in_channels * 2, in_channels, kernel_size=1, stride=1, padding=0, bias=False)
#
#         self.gelu = nn.GELU()
#         self.is_train = is_train  # 标记是否已经重参数化
#
#     def forward(self, x, x_last):
#         if self.is_train:
#             # 训练阶段：使用原始的多分支结构
#             x = self.in_conv1x1(x)
#             x = self.expand_conv1x1(x)
#             x = self.gelu(x)
#
#             # 多尺度卷积和Identity相加
#             x = self.dwconv_k1(x) + self.dwconv_k3(x) + self.dwconv_k5(x) + self.identity(x)
#
#             x = self.out_conv1x1(x)
#             x = self.reduce_conv1x1(x)
#
#             x = x + x_last
#         else:
#             # 推理阶段：使用重参数化后的层
#             x = self.fused_in_conv(x)
#             x = self.gelu(x)
#             x = self.fused_dwconv(x)
#             x = self.fused_out_conv(x)
#             x = x + x_last
#         return x
#
#     @torch.no_grad()
#     def fuse(self):
#         # 重参数化输入和输出的1x1卷积
#         fused_in_conv = nn.Conv3d(self.in_conv1x1.in_channels, self.expand_conv1x1.out_channels, kernel_size=1,
#                                   stride=1, padding=0, bias=False)
#         fused_in_conv.weight.data = (
#             torch.matmul(
#                 self.in_conv1x1.weight.data.view(self.in_conv1x1.in_channels, -1),
#                 self.expand_conv1x1.weight.data.view(self.in_conv1x1.out_channels, -1))
#             .view(fused_in_conv.weight.shape))
#         # fused_in_conv.bias.data = self.expand_conv1x1.bias.data + self.in_conv1x1.bias.data
#
#         # 重参数化多尺度卷积
#         fused_dwconv = nn.Conv3d(self.dwconv_k1.in_channels, self.dwconv_k1.out_channels, kernel_size=5, stride=1,
#                                  padding=2, groups=self.dwconv_k1.in_channels, bias=False)
#         fused_dwconv.weight.data = (F.pad(self.dwconv_k1.weight.data, [2, 2, 2, 2, 2, 2]) +
#                                     F.pad(self.dwconv_k3.weight.data, [1, 1, 1, 1, 1, 1]) +
#                                     self.dwconv_k5.weight.data)
#         # fused_dwconv.bias.data = torch.zeros_like(fused_dwconv.bias)  # 假设深度卷积的bias都为0
#
#         # 重参数化输出的两个1x1卷积
#         fused_out_conv = nn.Conv3d(self.out_conv1x1.in_channels, self.reduce_conv1x1.out_channels, kernel_size=1,
#                                    stride=1, padding=0, bias=False)
#         fused_out_conv.weight.data = torch.matmul(
#             self.out_conv1x1.weight.data.view(self.out_conv1x1.in_channels, -1),
#             self.reduce_conv1x1.weight.data.view(self.out_conv1x1.out_channels, -1)).view(fused_out_conv.weight.shape)
#         # fused_out_conv.bias.data = self.reduce_conv1x1.bias.data + self.out_conv1x1.bias.data
#
#         # 将重参数化后的卷积层赋值给模块并切换到推理模式
#         self.fused_in_conv = fused_in_conv
#         self.fused_dwconv = fused_dwconv
#         self.fused_out_conv = fused_out_conv
#         # self.fused = True  # 标记为已重参数化


class lightweight_module(nn.Module):
    def __init__(self, in_dim, out_dim, is_train, ffn_expansion_factor=2.66,):
        super(lightweight_module, self).__init__()
        self.norm = nn.LayerNorm(in_dim)

        hidden_features = int(in_dim * ffn_expansion_factor)

        self.project_in = nn.Conv3d(in_dim, hidden_features * 2, kernel_size=1)
        ...
        self.project_out = nn.Conv3d(hidden_features, out_dim, kernel_size=1)

    def forward(self, x, x_last):
        x = x.permute(0, 2, 3, 4, 1)
        # x = self.ln1(x)
        # x = x_last + self.conv(self.branch1(x) * self.branch2(x))
        x = self.norm(x)
        x = x.permute(0, 4, 1, 2, 3)
        x = self.project_in(x)
        x1, x2 = self.dwconv(x).chunk(2, dim=1)
        x = self.act_fn(x1) * x2
        x = self.project_out(x) + x_last
        return x
...
class IPB(nn.Module):
    def __init__(self, in_dim=8, out_dim=4, dim=4, is_train=True):
        ...
        self.reuseFusion2 = nn.Conv3d(dim * 6, dim * 2, 1, 1, 0, bias=False)
        ...
        self.reuseFusion1 = nn.Conv3d(dim * 3, dim, 1, 1, 0, bias=False)
        ...
        # self.downReuse1 = lightweight_module(dim, dim)
        # self.downReuse2 = lightweight_module(dim * 2, dim * 2)
        # self.bottleneck_localReuse = lightweight_module(dim * 2, dim * 2)
        # self.bottleneck_swinReuse = lightweight_module(dim * 2, dim * 2)
        # self.upReuse2 = lightweight_module(dim * 2, dim * 2)
        # self.upReuse1 = lightweight_module(dim, dim)

        self.downReuse1 = lightweight_module(dim, dim, is_train)
        self.downReuse2 = lightweight_module(dim * 2, dim * 2, is_train)
        self.bottleneck_localReuse = lightweight_module(dim * 2, dim * 2, is_train)
        self.bottleneck_swinReuse = lightweight_module(dim * 2, dim * 2, is_train)
        self.upReuse2 = lightweight_module(dim * 2, dim * 2, is_train)
    ...
    def reparameterize(self):
        # 调用子模块的重参数化方法
        self.downReuse1.fuse()
        self.downReuse2.fuse()
        self.bottleneck_localReuse.fuse()
        self.bottleneck_swinReuse.fuse()
        self.upReuse2.fuse()
        self.upReuse1.fuse()
    ...
    def forward(self, x, stage_ins=None, skip=False, stage_outs=None):
        ...
        if not skip:
            ...
            x = self.downsample1(stage_outs[0])
            x2 = self.down2(x)
            if stage_ins:
            ...
            x_swin = self.bottleneck_swin(x[:, c:, :, :] + stage_outs[2])
            if stage_ins:
            ...
            x2 = self.downReuse2(x, stage_ins[1])
            stage_outs.append(self.down2_action(x2, stage_ins[1], stage_ins[4]))
            x2 = stage_outs[1]
            x = self.downsample2(stage_outs[1])

            x_local = self.bottleneck_localReuse(x[:, :c, :, :], stage_ins[2])
            stage_outs.append(self.bottleneck_local_action(x_local, stage_ins[2], stage_ins[3]))

            x_swin = self.bottleneck_swinReuse(x[:, c:, :, :] + stage_outs[2], stage_ins[3])
            stage_outs.append(self.bottleneck_swin_action(x_swin, stage_ins[3], stage_ins[2]))
            ...
            x = self.upsample2(x)
            x = x2 + self.fusion2(torch.cat([x, x2], dim=1))
            x = self.upReuse2(x, stage_ins[4])
            stage_outs.append(self.up2_action(x, stage_ins[4], stage_ins[1]))

            x = self.upsample1(stage_outs[4])
            x = x1 + self.fusion1(torch.cat([x, x1], dim=1))
            x = self.upReuse1(x, stage_ins[5])
            stage_outs.append(self.up1_action(x, stage_ins[5], stage_ins[0]))
...
class NetVideo_base_resnet_GDFN(torch.nn.Module):
    def __init__(self, opt):
        super(NetVideo_base_resnet_GDFN, self).__init__()
        ...
        self.net_stage_head = nn.ModuleList(nn.ModuleList([
            IPB(in_dim=opt.dim * 2, out_dim=opt.dim, dim=opt.dim, is_train=opt.is_train),
    ...
    def reparameterize_all(self):
        # 调用子模块的重参数化方法
        self.net_stage_head[0].reparameterize()
        self.net_stage_body[0].reparameterize()
        self.net_stage_tail[0].reparameterize()
    ...
    def forward(self, g, input_mask=None):
        ...
        if not self.body_share_params:
            for i in range(self.stage - 2):
                mu = self.mu[i + 1](f)
                z, stage_output = self.net_stage_body[i][0](torch.cat([f + y / mu, f], dim=1), stage_output)
            ...
            for i in range(self.stage - 2):
                ...
                if i % 2 == 0:
                    ...
                    z, stage_output = self.net_stage_body[0](torch.cat([f + y / mu, f], dim=1), stage_output)
        ...
        z, stage_output = self.net_stage_tail[0](torch.cat([f + y / mu, f], dim=1), stage_output)
