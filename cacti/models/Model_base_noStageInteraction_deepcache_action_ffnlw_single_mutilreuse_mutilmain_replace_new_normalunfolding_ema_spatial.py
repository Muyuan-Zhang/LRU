...
class FAB_TSAB_inter(nn.Module):
    def __init__(self, dim, sq_dim, window_size=(8, 8), dim_head=28, mult=4, shift=False):
        super().__init__()
        self.FAB = FAB_inter(dim=dim, sq_dim=sq_dim, window_size=window_size, dim_head=dim_head, mult=mult, shift=shift)
        self.TSAB = TSAB(dim, num_head=dim_head)
        # self.TSAB2 = TSAB(dim, num_head=dim_head)
...
class TimesAttention3D_inter(nn.Module):
    ...
    def forward(self, x, last):
        ...
        kv = kv.reshape(n, B, 2, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        # C = C // 2
        k, v = kv[0], kv[1]
...
class NetVideo_base_noStageInteraction_deepcache_action_ffnlw_single_multireuse_multimain_replace_new_normalunfolding_ema_spatial(
    torch.nn.Module):
    def __init__(self, opt):
        super(
            NetVideo_base_noStageInteraction_deepcache_action_ffnlw_single_multireuse_multimain_replace_new_normalunfolding_ema_spatial,
            self).__init__()
