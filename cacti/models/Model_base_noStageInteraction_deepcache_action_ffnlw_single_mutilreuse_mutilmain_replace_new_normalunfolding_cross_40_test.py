...
class FAB_TSAB_inter(nn.Module):
    def __init__(self, dim, sq_dim, window_size=(8, 8), dim_head=28, mult=4, shift=False):
        super().__init__()
        self.fusion = nn.Conv3d(dim * 2, dim, kernel_size=1, stride=1, padding=0, bias=False)
        self.SF1 = StageInteraction(dim)
        ...
        # self.TSAB2 = TSAB(dim, num_head=dim_head)
        self.SF2 = StageInteraction(dim)

    def forward(self, x, enc, dec):
        x = self.FAB_inter(x, enc)
        x = self.TSAB_inter(x, enc)
        enc = self.SF1(x, enc, enc)
        x = enc + self.fusion(torch.cat([dec, x], dim=1))
        x = self.SF2(x, dec, dec)
        # x = self.TSAB2(x)
        return x, enc
...
class TSAB_inter(nn.Module):
    def __init__(self, dim, num_head):
        ...
        # self.conv = nn.Conv3d(dim, dim, kernel_size=1, stride=1, padding=0, bias=False)
        # self.act_fn = nn.LeakyReLU()
        # self.norm2 = nn.LayerNorm(dim)
        # self.mpmlp = MPMLP(dim=dim, multi=4)
        # self.ffn = FeedForward(dim=dim)
...
class IPB_without(nn.Module):
    ...
    def forward(self, x, stage_ins=None, skip=False, stage_outs=None):
        ...
        if not skip:
            ...
            x = self.down1(x)
            # x1 = self.downReuse1(x, stage_ins[0])
            x, enc = self.down1_cross(x, stage_ins[0], stage_ins[1])
...
class NetVideo_base_noStageInteraction_deepcache_action_ffnlw_single_multireuse_multimain_replace_new_normalunfolding_cross_40_test(
    torch.nn.Module):
    def __init__(self, opt):
        super(
            NetVideo_base_noStageInteraction_deepcache_action_ffnlw_single_multireuse_multimain_replace_new_normalunfolding_cross_40_test,
            self).__init__()
