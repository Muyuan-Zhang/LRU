...
class IPB_without(nn.Module):
    ...
    def forward(self, x, stage_ins=None, skip=False, stage_outs=None):
        ...
        if not skip:
            ...
            x = x1 + self.fusion1(torch.cat([stage_ins[0], x1], dim=1))
            # x = self.up1(x)
            # x = self.upReuse1(x, stage_ins[1])
...
class NetVideo_base_noStageInteraction_deepcache_action_ffnlw_single_multireuse_multimain_replace_new_normalunfolding_cross_29_noin(
    torch.nn.Module):
    def __init__(self, opt):
        ...
        self.vrm = nn.Sequential(
            nn.Conv3d(opt.dim, 1, kernel_size=1, stride=1, padding=0),
            # nn.LeakyReLU(inplace=True),
