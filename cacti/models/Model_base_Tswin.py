...
class FA(nn.Module):
    def __init__(self, dim, window_size=(8, 8, 2), dim_head=28, sq_dim=None, shift=True):
        super().__init__()
        ...
        num_token = window_size[0] * window_size[1] * window_size[2]
        self.cal_atten = Attention(dim_head, num_token)
    ...
    def forward(self, x):
        ...
        w_size = self.window_size
        if self.shift:
            x = x.roll(shifts=4, dims=1).roll(shifts=4, dims=2).roll(shifts=1, dims=3)
        x_inp = rearrange(x, 'b (B b2) (h b0) (w b1) c -> (b h w B) (b0 b1 b2) c',
                          b0=w_size[0], b1=w_size[1], b2=w_size[2])
        out = self.cal_attention(x_inp)
        out = rearrange(out, '(b h w B) (b0 b1 b2) c -> b (B b2) (h b0) (w b1) c',
                        h=h // w_size[0], w=w // w_size[1], b0=w_size[0], b1=w_size[1], b2=w_size[2], B=B // w_size[2])
        if self.shift:
            out = out.roll(shifts=-4, dims=1).roll(shifts=-4, dims=2).roll(shifts=-1, dims=3)
        return out
...
#         return x
class FAB(nn.Module):
    def __init__(self, dim, sq_dim, window_size=(8, 8, 2), dim_head=28, mult=4, shift=False):
        super().__init__()
        self.pos_emb = nn.Conv3d(dim, dim, 5, 1, 2, bias=False, groups=dim)
        self.norm1 = nn.LayerNorm(dim)
        self.fa = FA(dim=dim, window_size=window_size, dim_head=dim_head, sq_dim=sq_dim, shift=shift)
...
class NetVideo_base_Tswin(torch.nn.Module):
    def __init__(self, opt):
        super(NetVideo_base_Tswin, self).__init__()
