...
class DA(nn.Module):
    def __init__(self, dim, window_size=(8, 8), dim_head=28, sq_dim=None, shift=True):
        ...
        self.to_out = nn.Conv3d(dim, dim, 1, 1, bias=False)

        self.ta = TimesAttention3D(dim, dim_head)
...
class SCAB(nn.Module):
    def __init__(self, dim, sq_dim, window_size=(8, 8), dim_head=28, mult=4, shift=False):
        ...
        self.fa = CrossPreNorm(dim,
                               ...
                               norm_type='ln')
        # self.CA = ChanelAttention(dim, dim)
        # self.ffn = CrossPreNorm(dim, CrossFFN(dim, mult=mult), norm_type='ln')
        # self.fusion = FusionSpatialBlock(in_dim=dim, out_dim=dim)
...
class DSSCTAB(nn.Module):
    def __init__(self, dim, num_head, mult):
        ...
        self.ffn = CrossPreNorm(dim, CrossFFN(dim, mult), norm_type='ln')
...
class DSSCAB(nn.Module):
    def __init__(self, dim, dim_head=28):
        super().__init__()
        self.DSSCSAB1 = DSSCSAB(dim=dim, sq_dim=dim, dim_head=dim, mult=2)
        # self.DSSCSAB2 = DSSCSAB(dim=dim, sq_dim=dim, dim_head=dim, mult=2)
        self.fusion_block1 = FusionBlock(in_dim=dim, out_dim=dim)
        self.fusion_block2 = FusionBlock(in_dim=dim, out_dim=dim)
        self.fusion_block3 = FusionBlock(in_dim=dim, out_dim=dim)
        self.fusion_block4 = FusionBlock(in_dim=dim, out_dim=dim)
        self.DSSCTAB1 = DSSCTAB(dim, num_head=dim_head, mult=2)
        # self.DSSCTAB2 = DSSCTAB(dim, num_head=dim_head, mult=2)
    ...
    def forward(self, x, stage_in):
        stage_outs = []

        x = self.fusion_block1(x, stage_in[0])
        stage_outs.append(x)
        # x = self.DSSCTAB1(x, stage_in[1])
        x = self.fusion_block2(x, stage_in[1])
        stage_outs.append(x)
        # x = self.DSSCSAB2(x, stage_in[2])
        x = self.fusion_block3(x, stage_in[2])
        stage_outs.append(x)
        x = self.DSSCSAB1(x, stage_in[3])
        x = self.DSSCTAB1(x, stage_in[3])
        x = self.fusion_block4(x, stage_in[3])
        stage_outs.append(x)
        return x, stage_outs
...
class TCAB(nn.Module):
    def __init__(self, dim, num_head):
        ...
        # self.ffn = CrossPreNorm(dim, CrossFFN(dim), norm_type='ln')
...
class DSTCT(nn.Module):
    def __init__(self, in_dim=8, out_dim=4, dim=4):
        ...
        # self.main = STSAB(dim=dim, sq_dim=dim, dim_head=dim, mult=4)
    ...
    def forward(self, x, stage_ins=None, skip=False, stage_outs=None):
        ...
        x_in = x
        # x = self.main(x)
        x, stage_outs = self.reuse(x, stage_ins)
...
class FEM(nn.Module):
    def __init__(self, in_dim=1, dim=4):
...
class NetVideo_base_noStageInteraction_deepcache_action_ffnlw_single_multireuse_multimain_replace_new_normalunfolding_7(
    torch.nn.Module):
    def __init__(self, opt):
        ...
        # self.fem_phi = FEM(in_dim=1, dim=self.dim // 4)
        # self.fem_g = FEM(in_dim=1, dim=self.dim // 4)
        #
        # 特征恢复块
    ...
    def forward(self, g, input_mask=None):
        ...
        # 直接特征提取
        temp_g = temp_g.permute(0, 4, 1, 2, 3).repeat(1, self.dim, 1, 1, 1)  # 形状[bs, c 8, h, w]
        # temp_g = self.fem_g(temp_g.permute(0, 4, 1, 2, 3))  # 形状[bs, c 8, h, w]
        Phi = Phi.permute(0, 4, 1, 2, 3).repeat(1, self.dim, 1, 1, 1)  # 形状[bs, c 8, h, w]
        # Phi = self.fem_phi(Phi.permute(0, 4, 1, 2, 3))  # 形状[bs, c 8, h, w]
        temp_g = temp_g / (self.dim * 2)
        Phi = Phi / (self.dim * 2)
