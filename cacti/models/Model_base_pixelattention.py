...
class PA(nn.Module):
    def __init__(self, dim, window_size=(8, 8), dim_head=28, sq_dim=None, shift=True):
        ...
        self.q_spatial = nn.Conv3d(dim, dim, 1, 1, 0, bias=False, groups=dim)
        self.k_spatial = nn.Conv3d(dim, dim * 9, kernel_size=(1, 3, 3), stride=1, padding=(0, 1, 1), bias=False,
                                   groups=dim)
        # self.k_spatial = nn.Conv3d(dim, dim * 9, 1, 1, 0, bias=False, groups=dim)
        # self.v_spatial = nn.Conv3d(dim, dim * 9, kernel_size=(1, 3, 3), stride=1, padding=(0, 1, 1), bias=False,
        #                            groups=dim)
        self.v_spatial = nn.Conv3d(dim, dim * 9, 1, 1, 0, bias=False, groups=dim)

        self.proj_spa = nn.Linear(dim, dim)
    ...
    def forward(self, x):
        ...
        # pool_in = self.pool(x.permute(0, 4, 1, 2, 3))
        # adaptive_max_pool = nn.AdaptiveMaxPool3d(output_size=(h, w, 1))
        # pool_in = adaptive_max_pool(pool_in).permute(0, 4, 2, 3, 1)
        q_spatial = self.q_spatial(pool_in)
        k_spatial = self.k_spatial(pool_in)
        v_spatial = self.v_spatial(pool_in)
        # q_spatial = q_spatial.unsqueeze(1)
        q_spatial = rearrange(q_spatial, "b (h d m) B a w->b a w B h m d", h=self.heads_qk, m=1)
        k_spatial, v_spatial = map(lambda t: rearrange(t, 'b (h d m) B a w -> b a w B h m d',
                                                       h=self.heads_qk, m=self.M_size[0] * self.M_size[1]),
                                   (k_spatial, v_spatial))
        q_spatial = q_spatial * self.scale
        sim = einsum('b a w B h i d, b a w B h j d -> b a w B h i j', q_spatial, k_spatial)
        sim = self.softmax(sim)
        spatial_out = einsum('b a w B h i j, b a w B h j d -> b a w B h i d', sim, v_spatial)
        spatial_out = rearrange(spatial_out, "b a w B h i d -> b B a w (h i d)")
        x = self.proj_spa(spatial_out)
        # x = x.permute(0, 4, 1, 2, 3)
        return x
...
class PAB(nn.Module):
    def __init__(self, dim, sq_dim, window_size=(8, 8), dim_head=28, mult=4, shift=False):
        ...
        self.pos_emb = nn.Conv3d(dim, dim, 5, 1, 2, bias=False, groups=dim)
        self.norm1 = nn.LayerNorm(dim)
        self.pa = PA(dim=dim, window_size=window_size, dim_head=dim_head, sq_dim=sq_dim, shift=shift)
        self.norm2 = nn.LayerNorm(dim)
        # self.mpmlp = MPMLP(dim=dim, multi=mult)
        self.ffn = FeedForward(dim=dim, mult=mult)
...
class FAB_TSAB(nn.Module):
    def __init__(self, dim, sq_dim, window_size=(8, 8), dim_head=28, mult=4, shift=False):
        super().__init__()
        self.PAB = PAB(dim=dim, sq_dim=sq_dim, window_size=window_size, dim_head=dim_head, mult=mult, shift=shift)
...
class NetVideo_base_pixelattention(torch.nn.Module):
    def __init__(self, opt):
        super(NetVideo_base_pixelattention, self).__init__()
