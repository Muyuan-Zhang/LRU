...
class IPB_without(nn.Module):
    ...
    def forward(self, x, stage_ins=None, skip=False, stage_outs=None):
        ...
        if not skip:
            ...
            x = x1 + self.fusion1(torch.cat([stage_ins[1], x1], dim=1))
            x = self.down2_cross(x, stage_ins[0])
            # x = self.up1(x)
            # x = self.upReuse1(x, stage_ins[1])
            x = self.up1_action(x, stage_ins[1], stage_ins[0])
            stage_outs.append(x)
...
class NetVideo_base_noStageInteraction_deepcache_action_ffnlw_single_multireuse_multimain_replace_new_normalunfolding_cross_27(
    torch.nn.Module):
    def __init__(self, opt):
        super(
            NetVideo_base_noStageInteraction_deepcache_action_ffnlw_single_multireuse_multimain_replace_new_normalunfolding_cross_27,
            self).__init__()
