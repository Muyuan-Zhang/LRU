...
class FA(nn.Module):
    def __init__(self, dim, window_size=(8, 8), dim_head=28, sq_dim=None, shift=True):
        ...
        self.pool_to_qk = nn.Linear(dim, self.rank * 2, bias=False)
        self.pool_to_v = nn.Linear(dim, dim, bias=False)
        self.pool_to_out = nn.Linear(dim, dim)
    ...
    def cal_pool_attention(self, x):
        x = self.norm(x)
        q, k = self.pool_to_qk(x).chunk(2, dim=-1)
        v = self.pool_to_v(x)
        q, k = map(lambda t: rearrange(t, 'b n B (h d) -> b h B n d', h=self.heads_qk), (q, k))
        v = rearrange(v, 'b n B (h d) -> b h B n d', h=self.heads_v)
        attn = einsum('b h B i d, b h B j d -> b h B i j', q, k)
        attn = self.softmax(attn)
        out = einsum('b h B i j, b h B j d -> b h B i d', attn, v)
        out = rearrange(out, 'b h B n d -> b n B (h d)')
        out = out.repeat(1, 1, 8, 1)
        out = self.pool_to_out(out)
        return out
    ...
    def forward(self, x):
        ...
        if self.shift:
            ...
            pool_in = pool_in.roll(shifts=4, dims=2).roll(shifts=4, dims=3)
        x_inp = rearrange(x, 'b B (h b0) (w b1) c -> (b h w) (b0 b1) B c', b0=w_size[0], b1=w_size[1])
        pool_in = rearrange(pool_in, 'b B (h b0) (w b1) c -> (b h w) (b0 b1) B c', b0=w_size[0], b1=w_size[1])

        out = self.cal_attention(x_inp)
        pool_out = self.cal_pool_attention(pool_in)
        out = out + pool_out
        out = rearrange(out, '(b h w) (b0 b1) B c -> b B (h b0) (w b1) c',
...
class NetVideo_base_poolattention(torch.nn.Module):
    def __init__(self, opt):
        super(NetVideo_base_poolattention, self).__init__()
